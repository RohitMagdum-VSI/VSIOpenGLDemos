...
void initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode =
		...
		"out_texcoord.y = 1.0 - vTexCoords.y;"  \
		"out_normal = vNormal;" \
		"vec4 P = u_mv_matrix * vec4(vPosition, 1.0f);" \
		"gl_Position = u_projection_matrix * P;" \
		"}";
	...
	const GLchar* fragmentShaderSourceCode =
		...
		"void main(void)" \
		"{" \
		"FragColor = vec4(out_normal, 1.0f);" \
		"}";
	...
	gMVUniform = glGetUniformLocation(gShaderProgramObject, "u_mv_matrix");
	gProjectionMatrixUniform = glGetUniformLocation(gShaderProgramObject, "u_projection_matrix");
	gSamplerUniform = glGetUniformLocation(gShaderProgramObject, "u_sampler");

	bool bRet = gcModelLoader.LoadModel((CHAR *)"dwarf.x", &gModel);
	if (false == bRet)
	{
		fprintf(gpFile, "Error while LoadMode()\n");
		uninitialize();
		ExitProcess(EXIT_FAILURE);
	}
	...
	glBufferData(GL_ARRAY_BUFFER, gModel.vec3Vertices.size() * 3 * sizeof(GLfloat), &(gModel.vec3Vertices[0][0]), GL_STATIC_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, gModel.vec3Normals.size() * 3 * sizeof(GLfloat), &(gModel.vec3Normals[0][0]), GL_STATIC_DRAW);
...
void display(void)
{
	...
	viewMatrix = vmath::lookat(vmath::vec3(0.0f, 0.0f, 10.0f), vmath::vec3(0.0f, 0.0f, -1.0f), vmath::vec3(0.0f, 1.0f, 0.0f));
	modelMatrix = vmath::translate(0.0f, -3.0f, 0.0f);

	modelViewProjectionMatrix = viewMatrix * modelMatrix;

	glUniformMatrix4fv(gMVUniform, 1, GL_FALSE, modelViewProjectionMatrix);
	glUniformMatrix4fv(gProjectionMatrixUniform, 1, GL_FALSE, gPerspectiveProjectionMatrix);

	/*glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, m_TextureDwarf);
	glUniform1i(m_SampleLocation, 0); */

	//bind Vao
	glBindVertexArray(gVao);

	glDrawArrays(GL_TRIANGLES, 0, gModel.vec3Vertices.size());
