...
void initialize(void)
{
	...
	if (err != cudaSuccess)
	{
		wsprintf(str, L"GPU Memory Fatal Error = %s In File Name %s at Line No.%d\nExitting...\n", cudaGetErrorString(err), __FILE__, __LINE__);
...
void runCuda(void)
{
	...
	kernel(MESH_WIDTH, MESH_HEIGHT, dptr, gfAnimate);
...
void computeFPS(void)
{
	...
	wchar_t fps[256];
	wsprintf(fps, L"OpenGL CUDA Interoperability %3.1f fps", avgFPS);
