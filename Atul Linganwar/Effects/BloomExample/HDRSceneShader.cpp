...
bool InitializeHDRSceneShaderProgram(HDR_SCENE_SHADER* pShaderObj)
{
	...
	const GLchar* pchVertexShaderSource =
		...
		"uniform vec4 u_light_position = vec4(100.0f, 100.0f, 100.0f, 0.0f);\n" \
	...
	if (iShaderCompiledStatus == GL_FALSE)
	{
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				fprintf(gpFile, "HDR Scene VertexShader compilation log: %s\n", szInfoLog);
				free(szInfoLog);
				return false;
			}
		}
	...
	const GLchar* pchFragmentShaderSource =
		...
		"out vec4 FragColor2;\n" \
		"uniform float u_bloom_thresh_min = 0.8f;\n" \
		"uniform float u_bloom_thresh_max = 1.2f;\n" \
		"void main(void)\n" \
		"{\n" \
		...
		"FragColor1 = texColor * vec4(ads_light_color, 1.0);\n" \
		"float Y = dot(ads_light_color, vec3(0.299, 0.587, 0.144));\n" \
		"ads_light_color = ads_light_color * 4.0 * smoothstep(u_bloom_thresh_min, u_bloom_thresh_max, Y);\n" \
		"FragColor2 = texColor * vec4(ads_light_color, 1.0);\n" \
		"}\n";
	...
	if (iShaderCompiledStatus == GL_FALSE)
	{
		...
		if (iInfoLogLength > 0)
		{
			...
			if (szInfoLog != NULL)
			{
				...
				fprintf(gpFile, "HDR Scene FragmentShader compilation log: %s\n", szInfoLog);
				free(szInfoLog);
				return false;
			}
		}
	...
	pShaderObj->uiBloomThreshMinUniform = glGetUniformLocation(pShaderObj->ShaderObject.uiShaderProgramObject, "u_bloom_thresh_min");
	pShaderObj->uiBloomThreshMaxUniform = glGetUniformLocation(pShaderObj->ShaderObject.uiShaderProgramObject, "u_bloom_thresh_max");
