...
bool InitializeStarField(STAR_FIELD* pStarField)
{
	...
	glGenFramebuffers(1, &pStarField->uiFBO2D);
	glBindFramebuffer(GL_FRAMEBUFFER, pStarField->uiFBO2D);

	glGenTextures(1, &pStarField->uiStarFieldFB2DTexture);
	glBindTexture(GL_TEXTURE_2D, pStarField->uiStarFieldFB2DTexture);
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, pStarField->uiStarFieldFB2DTexture, 0);
	glBindTexture(GL_TEXTURE_2D, 0);
	
	glBindFramebuffer(GL_FRAMEBUFFER, 0);

	// FBO 3D
	glGenFramebuffers(1, &pStarField->uiFBO3D);
	glBindFramebuffer(GL_FRAMEBUFFER, pStarField->uiFBO3D);

	glGenTextures(1, &pStarField->uiStarFieldFB3DTexture);
	glBindTexture(GL_TEXTURE_3D, pStarField->uiStarFieldFB3DTexture);
	glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
	glTexImage3D(GL_TEXTURE_3D, 0, GL_RGBA, OGL_WINDOW_WIDTH, OGL_WINDOW_HEIGHT, 128, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
	glFramebufferTexture3D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_3D, pStarField->uiStarFieldFB3DTexture, 0, 0);
	glBindTexture(GL_TEXTURE_3D, 0);

	/*glGenRenderbuffers(1, &pStarField->uiStarFieldFB3DTextureDepth);
	glBindRenderbuffer(GL_RENDERBUFFER, pStarField->uiStarFieldFB3DTextureDepth);
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, OGL_WINDOW_WIDTH, OGL_WINDOW_HEIGHT);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, pStarField->uiStarFieldFB3DTextureDepth);
	glBindRenderbuffer(GL_RENDERBUFFER, 0);*/

	/*glGenTextures(1, &pStarField->uiStarFieldFB3DTextureDepth);
	glBindTexture(GL_TEXTURE_2D, pStarField->uiStarFieldFB3DTextureDepth);
...
{
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, pStarField->uiFBO2D);
...
{
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, pStarField->uiFBO3D);
...
void ResizeStarFieldFBO3D(STAR_FIELD* pStarField, int iWidth, int iHeight, int iDepth)
{
	if (pStarField->bIsStarFieldFBOInitialized)
	{
		...
		/*glBindTexture(GL_TEXTURE_3D, pStarField->uiStarFieldFB3DTextureDepth);
		glTexImage3D(GL_TEXTURE_3D, 0, GL_DEPTH_COMPONENT, iWidth, iHeight, iDepth, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
		glFramebufferTexture3D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_3D, pStarField->uiStarFieldFB3DTextureDepth, 0, 0);
		glBindTexture(GL_TEXTURE_3D, 0);*/
...
void UnInitializeStarField(STAR_FIELD* pStarField)
{
	...
	if (pStarField->uiFBO2D)
	{
		glDeleteBuffers(1, &pStarField->uiFBO2D);
		pStarField->uiFBO2D = 0;
	}

	if (pStarField->uiFBO3D)
	{
		glDeleteBuffers(1, &pStarField->uiFBO3D);
		pStarField->uiFBO3D = 0;
	}
}
