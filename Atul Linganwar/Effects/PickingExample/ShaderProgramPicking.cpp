...
bool ShaderProgram::Init()
{
	...
	case PICKING_COLOR_SHADER:
		bRet = InitializeColorProgramPicking();
...
bool ShaderProgram::InitializeTextureProgram(void)
{
	...
	LoadTexture(&m_uiTextureSmiley, MAKEINTRESOURCE(IDBITMAP_SMILEY));
...
bool ShaderProgram::InitializeColorProgramPicking(void)
{
	// Create vertex shader
	m_uiVertexShaderObject = glCreateShader(GL_VERTEX_SHADER);
	...
	const GLchar* glchFragmentShaderSource =
		...
		"void main(void)" \
		"{" \
		/*"FragColor = vec4(float(u_object_index_uniform), 0.0, 0.0, 1.0);" \*/
	...
	// get mvp uniform location
	m_uiMVPUniform = glGetUniformLocation(m_uiShaderProgramObject, "u_mvp_matrix");
	m_uiObjectIndexUniform = glGetUniformLocation(m_uiShaderProgramObject, "u_object_index_uniform");

	return true;
}

void ShaderProgram::UnInitializeColorProgramPicking(void)
{
	glDetachShader(m_uiShaderProgramObject, m_uiVertexShaderObject);
	glDetachShader(m_uiShaderProgramObject, m_uiFragmentShaderObject);
