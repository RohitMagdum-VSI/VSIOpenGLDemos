...
bool InitializeBasicQuadRTTTextureShaderProgram(BASIC_QUAD_RTT_TEXTURE_SHADER* pShaderObj)
{
	...
	const GLchar* pchFragmentShaderSource =
		...
		"void main(void)" \
		"{" \
		"vec4 Color = texture(u_texture0_sampler, out_texture0_coord);\n" \
		"if((Color.r < 0.01) && (Color.g < 0.01) && (Color.b < 0.01))\n" \
		"{\n" \
		"discard;\n" \
		"}\n" \
		"FragColor =  u_fade_out * Color;" \
		"}";
	...
	pShaderObj->uiFadeoutUniform = glGetUniformLocation(pShaderObj->ShaderObject.uiShaderProgramObject, "u_fade_out");
