...
GLfloat VeinColor[] = { 0.79f, 0.80f, 0.81f };
GLfloat MarbalColor[] = { 0.31f, 0.29f, 0.31f };
...
bool Initialize()
{
	...
	const GLchar* glchFragmentShaderSource =
		...
		"void main(void)\n" \
		"{\n" \
		"vec4 noisevec = texture(Noise, (Scale * MCposition) + offset);\n" \
...
void Display(void)
{
	...
	RotateMatrix = vmath::rotate(gfAngle, 0.0f, 1.0f, 1.0f);

	modelViewMatrix *= RotateMatrix;
	...
	glUniform3fv(guiVeinColorUniform, 1, VeinColor);
	(glUniform3fv(guiMarbalColorUniform,1, MarbalColor));

	glUniform1f(guiScaleUniform, gfScale);
	glUniform1f(guiNoiseScaleUniform, gfNoiseScale);
	glUniform1f(guiSineFactoryUniform, gfSineFactor);
	glUniform1f(guiSineIntensityFactorUniform, gfSineIntensityFactor);
	glUniform1f(guiLightIntensityUniform, gfLightIntensity);
