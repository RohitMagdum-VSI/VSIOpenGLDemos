...
GLuint guiTextureSamplerUniformFont;
...
bool Initialize()
{
	...
	// Initialize FontMap
	if (false == InitializeFontMap("arial.ttf"))
	{
	...
	const GLchar* glchVertexShaderSource =
		...
		"void main(void)" \
		"{" \
		"gl_Position = u_mvp_matrix * vec4(vPosTexCoord.xy, 0.0, 1.0);" \
	...
	glBindAttribLocation(guiShaderProgramObject, OGL_ATTRIBUTE_POSITION, "vPosTexCoord");
	...
	guiTextureSamplerUniformFont = glGetUniformLocation(guiShaderProgramObject, "u_texture0_sampler");
	guiMVPUniform = glGetUniformLocation(guiShaderProgramObject, "u_mvp_matrix");

	glGenVertexArrays(1, &guiVAOFont);
	glBindVertexArray(guiVAOFont);

	glGenBuffers(1, &guiVBOFont);
	glBindBuffer(GL_ARRAY_BUFFER, guiVBOFont);
...
void Display(void)
{
	...
	glUniform1i(guiTextureSamplerUniformFont, 0);
	...
	RenderFont(
		guiShaderProgramObject, 
		guiVBOFont, 
		"Hello World", 
		color, -1.0f, 3.0f, 0.009f
	);
...
void UnInitialize()
{
	...
	if (0 != guiVBOFont)
	{
		glDeleteBuffers(1, &guiVBOFont);
		guiVBOFont = 0;
	}

	if (0 != guiVAOFont)
	{
		glDeleteBuffers(1, &guiVAOFont);
		guiVAOFont = 0;
	}
