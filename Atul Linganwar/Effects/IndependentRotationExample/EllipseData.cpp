...
PELLIPSE_DATA GetEllipseData(GLint iVerticesCount, GLint iSemiMajorAxis, GLint iSemiMinorAxis)
{
	...
	pEllipseData->pfVertices = (GLfloat*)calloc(3 * (iVerticesCount + 1), sizeof(GLfloat));
	...
	pEllipseData->iVerticesSize = 3 * (iVerticesCount + 1) * sizeof(GLfloat);
	...
	for (int i = 0; i < iVerticesCount + 1; i++)
	{
		...
		pEllipseData->pfVertices[iPosIndex++] = 0.0f;
