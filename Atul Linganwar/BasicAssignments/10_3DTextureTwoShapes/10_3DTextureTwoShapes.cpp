...
#include "10_3DTextureTwoShapes.h"
...
GLuint guiTextureKundali;
GLuint guiTextureStone;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	WCHAR wszClassName[] = L"3D Texture Two Shapes";
...
bool Initialize()
{
	...
	// Triangle 
	glGenVertexArrays(1, &guiVAOPyramid);
	glBindVertexArray(guiVAOPyramid);
	...
	LoadTexture(&guiTextureKundali, MAKEINTRESOURCE(IDBITMAP_KUNDALI));
	LoadTexture(&guiTextureStone, MAKEINTRESOURCE(IDBITMAP_STONE));
...
void Display(void)
{
	...
	rotationMatrix = vmath::rotate(gRotateAnglePyramid, 0.0f, 1.0f, 0.0f);
	...
	glBindTexture(GL_TEXTURE_2D, guiTextureStone);
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, guiTextureKundali);
	glUniform1i(guiTextureSamplerUniform, 0);
...
void UnInitialize()
{
	...
	if (0 != guiVAOPyramid)
	{
		glDeleteVertexArrays(1, &guiVAOPyramid);
		guiVAOPyramid = 0;
	}
