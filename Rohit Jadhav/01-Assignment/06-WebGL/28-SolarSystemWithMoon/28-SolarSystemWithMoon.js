...
function main(){
	...
	canvas_RRJ = document.getElementById("28-SolarSystemWithMoon-RRJ");
	if(!canvas_RRJ){
...
function draw(){
	...
	gl_RRJ.useProgram(shaderProgramObject_RRJ);
		...
		mat4.translate(modelViewMatrix_RRJ, modelViewMatrix_RRJ, [0.0, 0.0, -6.0]);
		...
		mat4.rotateY(modelViewMatrix_RRJ, modelViewMatrix_RRJ, degToRad(moon_RRJ));
		mat4.translate(modelViewMatrix_RRJ, modelViewMatrix_RRJ, [1.0, 0.0, 0.0]);


		mat4.scale(modelViewMatrix_RRJ, modelViewMatrix_RRJ, [0.1, 0.1, 0.1]);
		mat4.rotateX(modelViewMatrix_RRJ, modelViewMatrix_RRJ, degToRad(90.0));
		...
		gl_RRJ.bindVertexArray(vao_Sphere_RRJ);
			...
			//gl_RRJ.drawArrays(gl_RRJ.POINTS, 0, 30*30);
			gl_RRJ.vertexAttrib3f(WebGLMacros.AMC_ATTRIBUTE_COLOR, 0.250, 0.250, 0.250);
			gl_RRJ.bindBuffer(gl_RRJ.ELEMENT_ARRAY_BUFFER, vbo_Sphere_Index_RRJ);
			gl_RRJ.drawElements(gl_RRJ.TRIANGLES, (STACK_RRJ) * (SLICES_RRJ) * 6, gl_RRJ.UNSIGNED_SHORT, 0);
