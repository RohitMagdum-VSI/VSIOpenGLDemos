package com.rohit_r_jadhav.robotic_arm;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnDoubleTapListener, OnGestureListener{
	...
	final int SHOULDER_RRJ = 1;
	final int ELBOW_RRJ = 2;

	final int CLKWISE_RRJ = 3;
	final int ANTICLKWISE_RRJ = 4;

	int iWhichRotation_RRJ = SHOULDER_RRJ;
	...
	public boolean onSingleTapConfirmed(MotionEvent e){
		...
		if(iWhichRotation_RRJ == SHOULDER_RRJ)
			iWhichRotation_RRJ = ELBOW_RRJ;
		else if(iWhichRotation_RRJ == ELBOW_RRJ)
			iWhichRotation_RRJ =SHOULDER_RRJ;
	...
	private void display(){
		...
		GLES32.glUseProgram(shaderProgramObject_RRJ);
			...
			Matrix.translateM(modelViewMatrix_RRJ, 0, 0.0f, 0.0f, -12.0f);
			Matrix.rotateM(modelViewMatrix_RRJ, 0, shoulder_RRJ, 0.0f, 0.0f, 1.0f);
			Matrix.translateM(modelViewMatrix_RRJ, 0, 1.0f, 0.0f, 0.0f);

			my_glPushMatrix(modelViewMatrix_RRJ);

			Matrix.scaleM(modelViewMatrix_RRJ, 0, 2.0f, 0.5f, 1.0f);
				...
				GLES32.glBindVertexArray(vao_Sphere_RRJ[0]);
					GLES32.glVertexAttrib3f(GLESMacros.AMC_ATTRIBUTE_COLOR, 0.50f, 0.350f, 0.05f);
			...
			Matrix.translateM(modelViewMatrix_RRJ, 0, 1.0f, 0.0f, 0.0f);
			Matrix.rotateM(modelViewMatrix_RRJ, 0, elbow_RRJ, 0.0f, 0.0f, 1.0f);
			Matrix.translateM(modelViewMatrix_RRJ, 0, 1.0f, 0.0f, 0.0f);
	...
	private void update(){
		...
		if(iWhichRotation_RRJ == SHOULDER_RRJ){
			if(iWhichDirection_RRJ == CLKWISE_RRJ)
				shoulder_RRJ = (shoulder_RRJ + 1) % 360;
			else
				shoulder_RRJ = (shoulder_RRJ - 1) % 360;
		}
		else if(iWhichRotation_RRJ == ELBOW_RRJ){
			if(iWhichDirection_RRJ == CLKWISE_RRJ)
				elbow_RRJ = (elbow_RRJ + 1) % 360;
			else
				elbow_RRJ = (elbow_RRJ - 1) % 360;
