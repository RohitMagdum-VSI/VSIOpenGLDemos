package com.rohit_r_jadhav.geometry_shader;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener{
	...
	private int geometryShaderObject_RRJ;
	...
	private void initialize(){
		...
		/********** Geometry Shader **********/
		geometryShaderObject_RRJ = GLES32.glCreateShader(GLES32.GL_GEOMETRY_SHADER);
		final String geometryShaderSourceCode_RRJ = String.format(
		...
		GLES32.glShaderSource(geometryShaderObject_RRJ, geometryShaderSourceCode_RRJ);

		GLES32.glCompileShader(geometryShaderObject_RRJ);

		iShaderCompileStatus_RRJ[0] = 0;
		iInfoLogLength_RRJ[0] = 0;
		szInfoLog_RRJ = null;

		GLES32.glGetShaderiv(geometryShaderObject_RRJ, GLES32.GL_COMPILE_STATUS, iShaderCompileStatus_RRJ, 0);
		if(iShaderCompileStatus_RRJ[0] == GLES32.GL_FALSE){
			GLES32.glGetShaderiv(geometryShaderObject_RRJ, GLES32.GL_INFO_LOG_LENGTH, iInfoLogLength_RRJ, 0);
			if(iInfoLogLength_RRJ[0] > 0){
				szInfoLog_RRJ = GLES32.glGetShaderInfoLog(geometryShaderObject_RRJ);

				System.out.println("RTR: Geometry Shader Compilation Error: " + szInfoLog_RRJ);
		...
		GLES32.glAttachShader(shaderProgramObject_RRJ, geometryShaderObject_RRJ);
	...
	private void uninitialize(){
		...
		if(shaderProgramObject_RRJ != 0){
			...
			GLES32.glUseProgram(shaderProgramObject_RRJ);
					...
					if(geometryShaderObject_RRJ != 0){
						GLES32.glDetachShader(shaderProgramObject_RRJ, geometryShaderObject_RRJ);
						GLES32.glDeleteShader(geometryShaderObject_RRJ);
