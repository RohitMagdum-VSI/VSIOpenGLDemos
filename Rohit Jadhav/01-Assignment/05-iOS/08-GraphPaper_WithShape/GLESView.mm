...
-(id)initWithFrame:(CGRect)frame{
    ...
    [self FillGridPosition: Grid_Position_RRJ];
        ...
        [self Calculation: Triangle_Position];
    ...
    glBindVertexArray(0);
        ...
        glBindVertexArray(vao_Rect_RRJ);
            ...
            glBufferData(GL_ARRAY_BUFFER, sizeof(Rect_Position), Rect_Position, GL_STATIC_DRAW);
            glVertexAttribPointer(AMC_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);
            glEnableVertexAttribArray(AMC_ATTRIBUTE_POSITION);
            glBindBuffer(GL_ARRAY_BUFFER, 0);

            /********** Color **********/
            glGenBuffers(1, &vbo_Rect_Color_RRJ);
            glBindBuffer(GL_ARRAY_BUFFER, vbo_Rect_Color_RRJ);
...
-(void)drawView:(id)sender{
    ...
    for (int i = 1; i <= 2; i++) {
        ...
        if (i == 1) {
            [self FillCircle_Position: Circle_Position withColor: Circle_Color withFlag: 1];
            //FillCircle_Position(Circle_Position, Circle_Color, 1);
        }
        else if (i == 2) {
            [self FillCircle_Position: Circle_Position withColor: Circle_Color withFlag: 2];
        ...
        //glPointSize(1.500f);
            glBindVertexArray(vao_Circle_RRJ);
                glBindBuffer(GL_ARRAY_BUFFER, vbo_Circle_Position_RRJ);
                glBufferData(GL_ARRAY_BUFFER, sizeof(Circle_Position), Circle_Position, GL_DYNAMIC_DRAW);
            glBindBuffer(GL_ARRAY_BUFFER, 0);


            glBindBuffer(GL_ARRAY_BUFFER, vbo_Circle_Color_RRJ);
                glBufferData(GL_ARRAY_BUFFER, sizeof(Circle_Color), Circle_Color, GL_DYNAMIC_DRAW);
                glBindBuffer(GL_ARRAY_BUFFER, 0);
...
-(void)FillCircle_Position: (GLfloat[]) arr withColor:(GLfloat[]) arrColor withFlag:(int)iFlag {
...
-(void)dealloc{
    ...
    if (vbo_Rect_Color_RRJ) {
        glDeleteBuffers(1, &vbo_Rect_Color_RRJ);
        vbo_Rect_Color_RRJ = 0;
    }
