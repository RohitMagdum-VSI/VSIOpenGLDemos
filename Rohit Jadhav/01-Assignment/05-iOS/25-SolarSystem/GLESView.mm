...
#define YEAR 1
#define DAY 2
int iWhichMovement_RRJ = YEAR;

#define CLK_RRJ 3
#define ANT_CLK_RRJ 4
int iWhichRotation_RRJ = CLK_RRJ;
...
-(void)onSingleTap:(UITapGestureRecognizer*)gr{
    ...
    if(iWhichMovement_RRJ == YEAR){
        if(iWhichRotation_RRJ == CLK_RRJ)
            year_RRJ = (year_RRJ + 20) % 360;
        else
            year_RRJ = (year_RRJ - 20) % 360;
    }
    else if(iWhichMovement_RRJ == DAY){
        if(iWhichRotation_RRJ == CLK_RRJ)
...
-(void)onDoubleTap:(UITapGestureRecognizer*)gr{
    ...
    if(iWhichRotation_RRJ == CLK_RRJ)
        iWhichRotation_RRJ = ANT_CLK_RRJ;
    else
        iWhichRotation_RRJ = CLK_RRJ;
}

-(void)onLongPress:(UILongPressGestureRecognizer*)gr{

    if(iWhichMovement_RRJ == YEAR)
        iWhichMovement_RRJ = DAY;
    else
        iWhichMovement_RRJ = YEAR;
