...
//For Movement
#define SHOULDER_RRJ 1
#define ELBOW_RRJ 2
int iWhichMovement_RRJ = SHOULDER_RRJ;

//For Rotation
#define CLKWISE_RRJ 3
#define ANTICLK_RRJ 4
int iWhichRotation_RRJ = CLKWISE_RRJ;
...
-(id)initWithFrame:(CGRect)frame{
    ...
    [self makeSphere: 0.50f Pos:sphere_Position_RRJ Normals:sphere_Normal_RRJ Elements:sphere_Index_RRJ];
...
-(void)drawView:(id)sender{
    ...
    glUseProgram(shaderProgramObject_RRJ);
        ...
        modelMatrix_RRJ = modelMatrix_RRJ * vmath::translate(0.0f, 0.0f, -12.0f);
        ...
        modelViewMatrix_RRJ = modelViewMatrix_RRJ * vmath::rotate((GLfloat)shoulder_RRJ, 0.0f, 0.0f, 1.0f);
        modelViewMatrix_RRJ = modelViewMatrix_RRJ * vmath::translate(1.0f, 0.0f, 0.0f);

        [self my_glPushMatrix: modelViewMatrix_RRJ];
        

        modelViewMatrix_RRJ = modelViewMatrix_RRJ * vmath::scale(2.0f, 0.5f, 1.0f);
        ...
        glBindVertexArray(vao_Sphere_RRJ);
        glVertexAttrib3f(AMC_ATTRIBUTE_COLOR, 0.50f, 0.350f, 0.05f);
        ...
        modelViewMatrix_RRJ = modelViewMatrix_RRJ * vmath::translate(1.0f, 0.0f, 0.0f);
        modelViewMatrix_RRJ = modelViewMatrix_RRJ * vmath::rotate((GLfloat)elbow_RRJ, 0.0f, 0.0f, 1.0f);
        modelViewMatrix_RRJ = modelViewMatrix_RRJ * vmath::translate(1.0f, 0.0f, 0.0f);
...
-(void)onSingleTap:(UITapGestureRecognizer*)gr{
    ...
    if(iWhichMovement_RRJ == SHOULDER_RRJ){
        if(iWhichRotation_RRJ == CLKWISE_RRJ)
            shoulder_RRJ = (shoulder_RRJ + 10) % 360;
        else
            shoulder_RRJ = (shoulder_RRJ - 10) % 360;
    }
    else if(iWhichMovement_RRJ == ELBOW_RRJ){
        if(iWhichRotation_RRJ == CLKWISE_RRJ)
            elbow_RRJ = (elbow_RRJ + 10) % 360;
        else
            elbow_RRJ = (elbow_RRJ - 10) % 360;
    }
...
-(void)onDoubleTap:(UITapGestureRecognizer*)gr{
    ...
    if(iWhichMovement_RRJ == SHOULDER_RRJ)
        iWhichMovement_RRJ = ELBOW_RRJ;
    else
        iWhichMovement_RRJ = SHOULDER_RRJ;
...
-(void)onLongPress:(UILongPressGestureRecognizer*)gr{
    ...
    if(iWhichRotation_RRJ == CLKWISE_RRJ)
        iWhichRotation_RRJ = ANTICLK_RRJ;
    else
        iWhichRotation_RRJ = CLKWISE_RRJ;
