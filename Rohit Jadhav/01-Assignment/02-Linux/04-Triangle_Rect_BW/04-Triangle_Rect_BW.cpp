...
void CreateWindow(void){
	...
	fprintf(gbFile, "Matching GLXFBConfig: %d\n", iNumberOfFBConfig);
	
	int bestFrameBufferConfig = -1;
	int bestNumberOfSample = -1;
	int worstFrameBufferConfig = -1;
	int worstNumberOfSample = -1;

	for(int i = 0; i < iNumberOfFBConfig; i++){
		pTempXVisualInfo = glXGetVisualFromFBConfig(gpDisplay, pGLXFBConfig[i]);
	
		if(pTempXVisualInfo){
			int sample, sampleBuffers;

			glXGetFBConfigAttrib(gpDisplay, pGLXFBConfig[i], GLX_SAMPLE_BUFFERS, &sampleBuffers);
			glXGetFBConfigAttrib(gpDisplay, pGLXFBConfig[i], GLX_SAMPLES, &sample);

			if(bestFrameBufferConfig < 0 || sampleBuffers && sample > bestNumberOfSample){
				bestNumberOfSample = sample;
				bestFrameBufferConfig = i;
			}

			if(worstFrameBufferConfig < 0 || sampleBuffers && sample < worstNumberOfSample){
				worstFrameBufferConfig = i;
				worstNumberOfSample = sample;
			}
		}
	...
	XStoreName(gpDisplay, gWindow, "04-Static_Triangle&Rectangle_BW");
...
void initialize(void){
	...
	if(!gGLXContext){
		fprintf(gbFile, "glXCreateContext() Failed!!\n");
		fprintf(gbFile, "Now Getting Best Given by System\n");
	...
	const char *szVertexShaderSourceCode = 
		"#version 450 core" \ 
		"\n" \ 
	...
	glShaderSource(vertexShaderObject, 1,
		(const char**)&szVertexShaderSourceCode, NULL);
...
void uninitialize(void){
	...
	if(gShaderProgramObject){
		glUseProgram(gShaderProgramObject);
			...
			glGetShaderiv(gShaderProgramObject, GL_ATTACHED_SHADERS, &iShaderCount);
