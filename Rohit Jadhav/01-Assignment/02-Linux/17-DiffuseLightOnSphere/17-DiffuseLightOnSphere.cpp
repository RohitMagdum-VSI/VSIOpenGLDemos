...
typedef GLXContext (*pGLXCreateContextAttribsARBProc)(Display*, GLXFBConfig, GLXContext, Bool, const int*);
pGLXCreateContextAttribsARBProc glXCreateContextAttribsARB_RRJ = NULL;
...
void CreateWindow(void){
	...
	XVisualInfo *tempXVisualInfo_RRJ = NULL;
	...
	for(int i = 0; i < iNumberOfFBConfig_RRJ; i++){
		tempXVisualInfo_RRJ = glXGetVisualFromFBConfig(gpDisplay_RRJ, pGLXFBConfig_RRJ[i]);
		if(tempXVisualInfo_RRJ){
		...
		XFree(tempXVisualInfo_RRJ);
		tempXVisualInfo_RRJ = NULL;
	}
	...
	XStoreName(gpDisplay_RRJ, gWindow_RRJ, "17-DiffuseLightOnSphere");
...
void ToggleFullScreen(void){
	...
	memset(&xev_RRJ, 0, sizeof(xev_RRJ));
...
void initialize(void){
	...
	glXCreateContextAttribsARB_RRJ = (pGLXCreateContextAttribsARBProc)glXGetProcAddressARB((GLubyte*)"glXCreateContextAttribsARB");
	...
	if(gGLXContext_RRJ == NULL){
		...
		fprintf(gbFile_RRJ, "Getting Best from system\n");
	...
	const GLchar *szVertexShaderSourceCode_RRJ =
		...
		"{" \
			"if(u_LKeyPress == 1) {"\
				...
				"diffuseColor = u_Ld * u_Kd * max(dot(tNorm, source), 0.0f);" \
			"}" \
	...
	LKeyPressUniform = glGetUniformLocation(gShaderProgramObject_RRJ, "u_LKeyPress");
