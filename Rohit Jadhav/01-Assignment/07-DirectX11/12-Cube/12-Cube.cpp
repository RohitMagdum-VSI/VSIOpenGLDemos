...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-12-Cube");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-12-Cube"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
 	...
 	/********** Vertex Buffer Pyramid_Position **********/
 	D3D11_BUFFER_DESC bufferDesc_Pyramid_Pos_RRJ;
 	ZeroMemory((void*)&bufferDesc_Pyramid_Pos_RRJ, sizeof(D3D11_BUFFER_DESC));

 	bufferDesc_Pyramid_Pos_RRJ.Usage = D3D11_USAGE_DYNAMIC;
 	bufferDesc_Pyramid_Pos_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_Position);
 	bufferDesc_Pyramid_Pos_RRJ.BindFlags = D3D11_BIND_VERTEX_BUFFER;
 	bufferDesc_Pyramid_Pos_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_Pyramid_Pos_RRJ, NULL, &gpID3D11Buffer_VertexBuffer_Cube_Position_RRJ);

 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: ID3D11Device::CreateBuffer() For Pyramid_Position Failed!!\n");
 		fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: ID3D11Device::CreateBuffer() For Pyramid_Position Done!!\n");
 		fclose(gbFile_RRJ);
 	}
 	...
 	memcpy(mappedSubresource_Pyramid_Position_RRJ.pData, cube_Position, sizeof(cube_Position));
 	...
 	/********** Vertex Buffer For Pyramid Color **********/
 	D3D11_BUFFER_DESC bufferDesc_Pyramid_Color_RRJ;
 	ZeroMemory((void*)&bufferDesc_Pyramid_Color_RRJ, sizeof(D3D11_BUFFER_DESC));

 	bufferDesc_Pyramid_Color_RRJ.Usage = D3D11_USAGE_DYNAMIC;
 	bufferDesc_Pyramid_Color_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_Color);
 	bufferDesc_Pyramid_Color_RRJ.BindFlags = D3D11_BIND_VERTEX_BUFFER;
 	bufferDesc_Pyramid_Color_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_Pyramid_Color_RRJ, NULL,
 		&gpID3D11Buffer_VertexBuffer_Cube_Color_RRJ);

 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: ID3D11::CreateBuffer() for Pyramid_Color Failed!!\n");
 		fclose(gbFile_RRJ);
 		return(hr_RRJ);
 	}
 	else{
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: ID3D11::CreateBuffer() for Pyramid_Color Done!!\n");
 		fclose(gbFile_RRJ);	
 	}



 	/********** Memory Map I/O For Pyramid_Color **********/
 	...
 	memcpy(mappedSubresource_Pyramid_Color_RRJ.pData, cube_Color, sizeof(cube_Color));
...
void display(void){
	...
	static float angle_Cube = 360.0f;
	...
	UINT offset_Color = 0;
	gpID3D11DeviceContext_RRJ->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Cube_Color_RRJ, &stride_Color, &offset_Color);
