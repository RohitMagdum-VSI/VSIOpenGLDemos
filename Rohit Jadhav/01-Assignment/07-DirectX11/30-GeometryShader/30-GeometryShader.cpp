...
ID3D11GeometryShader *gpID3D11GeometryShader_RRJ = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-30-GeometryShader");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-30-GeometryShader"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
 	...
 	ID3DBlob *pID3DBlob_GeometryShaderCode_RRJ = NULL;
 	pID3DBlob_Error_RRJ = NULL;


 	hr_RRJ = D3DCompile(
 		geometryShaderSourceCode_RRJ,
 		lstrlenA(geometryShaderSourceCode_RRJ) + 1,
 		"GS",
 		NULL,
 		D3D_COMPILE_STANDARD_FILE_INCLUDE,
 		"main",
 		...
 		0,
 		0,
 		&pID3DBlob_GeometryShaderCode_RRJ,
 		&pID3DBlob_Error_RRJ);
 	...
 	if(FAILED(hr_RRJ)){
 		...
 		if(pID3DBlob_Error_RRJ != NULL){
	 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
	 		fprintf_s(gbFile_RRJ, "ERROR: Geometry Shader Compilation\n");
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: Geometry Shader Compiled\n");
 		fclose(gbFile_RRJ);
 	}


 	hr_RRJ = gpID3D11Device_RRJ->CreateGeometryShader(
 		pID3DBlob_GeometryShaderCode_RRJ->GetBufferPointer(),
 		pID3DBlob_GeometryShaderCode_RRJ->GetBufferSize(),
 		NULL,
 		&gpID3D11GeometryShader_RRJ);


 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: CreateGeometryShader()\n");
 		fclose(gbFile_RRJ);
 		return(hr_RRJ);
 	}
 	else{
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: CreateGeometryShader()\n");
 		fclose(gbFile_RRJ);
 	}


 	gpID3D11DeviceContext_RRJ->GSSetShader(gpID3D11GeometryShader_RRJ, NULL, 0);
 	...
 	bufferDesc_Tri_Pos_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(tri_Pos);
 	bufferDesc_Tri_Pos_RRJ.BindFlags = D3D11_BIND_VERTEX_BUFFER;
 	bufferDesc_Tri_Pos_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_Tri_Pos_RRJ, NULL, &gpID3D11Buffer_VertexBuffer_Tri_Position_RRJ);
 	...
 	gpID3D11DeviceContext_RRJ->Map(gpID3D11Buffer_VertexBuffer_Tri_Position_RRJ, 0, 
 		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource_Tri_Position_RRJ);
 	memcpy(mappedSubresource_Tri_Position_RRJ.pData, tri_Pos, sizeof(tri_Pos));
 	gpID3D11DeviceContext_RRJ->Unmap(gpID3D11Buffer_VertexBuffer_Tri_Position_RRJ, NULL);
 	...
 	gpID3D11DeviceContext_RRJ->GSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_RRJ);
...
void uninitialize(void){
		...
		if(gpID3D11GeometryShader_RRJ){
			gpID3D11GeometryShader_RRJ->Release();
			gpID3D11GeometryShader_RRJ = NULL;
		}
