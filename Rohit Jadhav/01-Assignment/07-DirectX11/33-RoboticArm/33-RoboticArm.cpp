...
//For Hand
float shoulder_RRJ = 0.0f;;
float elbow_RRJ = 0.0f;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-33-RoboticArm");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-33-RoboticArm"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam){
	...
	switch(iMsg){
		...
		case WM_CHAR:
			switch(wParam){
				case 'S':
					shoulder_RRJ = shoulder_RRJ + (20.0f / 360.0f);
					break;
				...
				case 'E':
					elbow_RRJ = elbow_RRJ + (20.0f / 360.0f);
					break;
...
void display(void){
	...
	worldViewMatrix_RRJ = XMMatrixTranslation(0.0f, 0.0f, 12.0);

	my_glPushMatrix(worldViewMatrix_RRJ);

	worldViewMatrix_RRJ = XMMatrixRotationZ(shoulder_RRJ) * worldViewMatrix_RRJ;
	worldViewMatrix_RRJ = XMMatrixTranslation(1.0f, 0.0, 0.0f) * worldViewMatrix_RRJ;

	my_glPushMatrix(worldViewMatrix_RRJ);

	worldViewMatrix_RRJ = XMMatrixScaling(2.0f, 0.5f, 1.0f) * worldViewMatrix_RRJ;
	...
	constantBuffer_RRJ.Color = XMVectorSet(0.50f, 0.350f, 0.05f, 1.0f);
	...
	worldViewMatrix_RRJ = XMMatrixTranslation(1.0f, 0.0f, 0.0f) * worldViewMatrix_RRJ;
	worldViewMatrix_RRJ = XMMatrixRotationZ(elbow_RRJ) * worldViewMatrix_RRJ;
	worldViewMatrix_RRJ = XMMatrixTranslation(1.0f, 0.0f, 0.0f) * worldViewMatrix_RRJ;
