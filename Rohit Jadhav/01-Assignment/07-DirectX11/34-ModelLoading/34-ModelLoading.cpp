...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	MSG msg_RRJ;
	HWND hwnd_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-34-ModelLoading");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
		szName_RRJ,
		TEXT("Rohit_R_Jadhav-D3D-34-ModelLoading"),
		WS_OVERLAPPEDWINDOW,
		100, 100, 
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
		...
		/********** SPHERE INDEX **********
		D3D11_BUFFER_DESC bufferDesc_Model_Index_RRJ;
		ZeroMemory((void*)&bufferDesc_Model_Index_RRJ, sizeof(D3D11_BUFFER_DESC));

		bufferDesc_Model_Index_RRJ.Usage = D3D11_USAGE_DYNAMIC;
		bufferDesc_Model_Index_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(model_Elements_RRJ);
		bufferDesc_Model_Index_RRJ.BindFlags = D3D11_BIND_INDEX_BUFFER;
		bufferDesc_Model_Index_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

		hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(
			&bufferDesc_Model_Index_RRJ,
			NULL,
			&gpID3D11Buffer_IndexBuffer_Model_Index_RRJ
			);

		if(FAILED(hr_RRJ)){
			fopen_s(&gbFile_RRJ, gLogFileName_RRJ, "a+");
			fprintf_s(gbFile_RRJ, "ERROR: Model Index CreateBuffer()\n");
			fclose(gbFile_RRJ);
			return(hr_RRJ);
		}
		else{
			fopen_s(&gbFile_RRJ, gLogFileName_RRJ, "a+");
			fprintf_s(gbFile_RRJ, "SUCCESS: Model Index CreateBuffer()\n");
			fclose(gbFile_RRJ);	
		}
		...
		D3D11_MAPPED_SUBRESOURCE mappedSubresource_Model_Index_RRJ;
		ZeroMemory((void*)&mappedSubresource_Model_Index_RRJ, sizeof(D3D11_MAPPED_SUBRESOURCE));

		gpID3D11DeviceContext_RRJ->Map(gpID3D11Buffer_IndexBuffer_Model_Index_RRJ, 0,
			D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource_Model_Index_RRJ);

		memcpy(mappedSubresource_Model_Index_RRJ.pData, model_Elements_RRJ, sizeof(model_Elements_RRJ));

		gpID3D11DeviceContext_RRJ->Unmap(gpID3D11Buffer_IndexBuffer_Model_Index_RRJ, 0);
		*/
...
void uninitialize(void){
	...
	/*if(gpID3D11Buffer_IndexBuffer_Model_Index_RRJ){
		gpID3D11Buffer_IndexBuffer_Model_Index_RRJ->Release();
		gpID3D11Buffer_IndexBuffer_Model_Index_RRJ = NULL;
	}*/
...
void display(void){
	...
	angle_Model_RRJ = angle_Model_RRJ + 0.002f;
