...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-10-Color-Tri-Rect-WithCullingOff");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
		szName_RRJ,
		TEXT("Rohit_R_Jadhav-D3D-10-Color-Tri-Rect-WithCullingOff"),
		WS_OVERLAPPEDWINDOW,
		100, 100,
...
HRESULT initialize(void){
	...
	if(FAILED(hr_RRJ)){
		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
		fprintf_s(gbFile_RRJ, "\n");
		fprintf_s(gbFile_RRJ, "ERROR: Rasterizer State -> ID3D11Device::CreateRasterizerState() Failed!!\n");
		fclose(gbFile_RRJ);
		return(hr_RRJ);
	}
	else{
		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
		fprintf_s(gbFile_RRJ, "\n");
		fprintf_s(gbFile_RRJ, "SUCCESS: Rasterizer State -> ID3D11Device::CreateRasterizerState() Done!!\n");
		fclose(gbFile_RRJ);	
	}
