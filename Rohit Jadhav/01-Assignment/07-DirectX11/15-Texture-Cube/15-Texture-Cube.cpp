...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-15-Texture-Cube");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-15-Texture-Cube"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
 	...
 	/********** LOAD TEXTURE **********/
	hr_RRJ = LoadD3DTexture(L"Kundali.bmp", &gpID3D11ShaderResourceView_Cube_Texture_RRJ);
	if(FAILED(hr_RRJ)){
		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
