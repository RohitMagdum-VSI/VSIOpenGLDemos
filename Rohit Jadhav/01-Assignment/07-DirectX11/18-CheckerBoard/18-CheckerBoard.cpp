...
HRESULT LoadD3DTexture(void){
	...
	if(FAILED(hr_RRJ)){
 	...
 	gpID3D11DeviceContext_RRJ->Map(pID3D11Texture2D_RRJ, 0,
 		 D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource_RRJ);

 	memcpy(mappedSubresource_RRJ.pData, checkImageData, sizeof(checkImageData));

 	gpID3D11DeviceContext_RRJ->Unmap(pID3D11Texture2D_RRJ, 0);
