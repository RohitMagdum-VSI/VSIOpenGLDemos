...
char checkImageData[CHECK_IMAGE_HEIGHT * CHECK_IMAGE_WIDTH * 4];
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-18-CheckerBoard");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-18-CheckerBoard"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
 	...
 	bufferDesc_Rect_Pos_RRJ.ByteWidth = sizeof(float) * 3 * 4;
 	...
 	bufferDesc_Rect_Texcoord_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(rect_Texcoord);
...
HRESULT LoadD3DTexture(void){
	...
	/********** Texture DESC **********/
	D3D11_TEXTURE2D_DESC texture2D_RRJ;
	ZeroMemory((void*)&texture2D_RRJ, sizeof(D3D11_TEXTURE2D_DESC));

	texture2D_RRJ.Width = CHECK_IMAGE_WIDTH;
	texture2D_RRJ.Height = CHECK_IMAGE_HEIGHT;
	texture2D_RRJ.MipLevels = 1;
	texture2D_RRJ.ArraySize = 1;
	texture2D_RRJ.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	texture2D_RRJ.SampleDesc.Count = 1;
	texture2D_RRJ.SampleDesc.Quality = 0;
	texture2D_RRJ.Usage = D3D11_USAGE_DYNAMIC;
	texture2D_RRJ.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	texture2D_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	texture2D_RRJ.MiscFlags = 0;



	ID3D11Texture2D *pID3D11Texture2D_RRJ = NULL;


	D3D11_SUBRESOURCE_DATA d3dSubresourceData_RRJ;
	ZeroMemory((void*)&d3dSubresourceData_RRJ, sizeof(D3D11_SUBRESOURCE_DATA));

	d3dSubresourceData_RRJ.pSysMem = checkImageData;
	d3dSubresourceData_RRJ.SysMemPitch = CHECK_IMAGE_WIDTH * 4;




	hr_RRJ = gpID3D11Device_RRJ->CreateTexture2D(
		&texture2D_RRJ, 
		&d3dSubresourceData_RRJ, 
		&pID3D11Texture2D_RRJ);
	...
	D3D11_SHADER_RESOURCE_VIEW_DESC shaderResourceViewDesc_RRJ;
	ZeroMemory((void*)&shaderResourceViewDesc_RRJ, sizeof(D3D11_SHADER_RESOURCE_VIEW_DESC));


	shaderResourceViewDesc_RRJ.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
	shaderResourceViewDesc_RRJ.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2DMS;

	hr_RRJ = gpID3D11Device_RRJ->CreateShaderResourceView(
		pID3D11Texture2D_RRJ,
		&shaderResourceViewDesc_RRJ,
		&gpID3D11ShaderResourceView_Rect_Texture_RRJ);

	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: CreateShaderResourceView()\n");
 		fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: CreateShaderResourceView()\n");
 		fclose(gbFile_RRJ);
 	}
...
void makeCheckImage(void){
	...
	for(int i = 0; i < CHECK_IMAGE_HEIGHT; i++){
		for(int j = 0; j < CHECK_IMAGE_WIDTH; j++){
			...
			checkImageData[(i * CHECK_IMAGE_WIDTH * 4) + (j * 4) + 0] = (char)c;
			checkImageData[(i * CHECK_IMAGE_WIDTH * 4) + (j * 4) + 1] = (char)c;
			checkImageData[(i * CHECK_IMAGE_WIDTH * 4) + (j * 4) + 2] = (char)c;
			checkImageData[(i * CHECK_IMAGE_WIDTH * 4) + (j * 4) + 3] = (char)255;	
		}
	}
...
void display(void){
	...
	float CheckerBoard_Position[3 * 4];
	...
	for(int i = 0; i < 2; i++){
		...
		/********** Memory Mapped I/O **********/
	 	...
	 	memcpy(mappedSubresource_Rect_Position_RRJ.pData, CheckerBoard_Position, sizeof(CheckerBoard_Position));
		...
		translateMatrix_RRJ = XMMatrixTranslation(0.0f, 0.0f, 4.0f);
