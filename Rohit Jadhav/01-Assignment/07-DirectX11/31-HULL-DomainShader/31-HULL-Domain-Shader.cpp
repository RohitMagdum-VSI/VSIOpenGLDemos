...
ID3D11HullShader *gpID3D11HullShader_RRJ = NULL;
ID3D11DomainShader *gpID3D11DomainShader_RRJ = NULL;
ID3D11PixelShader *gpID3D11PixelShader_RRJ = NULL;


//For Line
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Line_Position_RRJ = NULL;

//For Segments
unsigned int guiNumberOfLineSegments_RRJ = 1;
...
//For Uniform
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer_HullShader_RRJ = NULL;
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer_DomainShader_RRJ = NULL;
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer_PixelShader_RRJ = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-31-HULL-DOMAIN-Shader");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-31-HULL-DOMAIN-Shader"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
	...
	switch(iMsg){
		...
		case WM_KEYDOWN:
			switch(wParam){
				...
				case VK_UP:
					guiNumberOfLineSegments_RRJ++;
					if(guiNumberOfLineSegments_RRJ > 30)
						guiNumberOfLineSegments_RRJ = 30;
					break;

				case VK_DOWN:
					guiNumberOfLineSegments_RRJ--;
					if(guiNumberOfLineSegments_RRJ < 1)
						guiNumberOfLineSegments_RRJ = 1;
					break;
			}
			break;
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ =
 	...
 	/*********** HULL SHADER **********/
 	const char *hullShaderSourceCode_RRJ = 
 		"cbuffer ConstantBuffer {" \
 			"float4 hull_constant_function_params;" \
 		"};" \


 		//For Geting Input From Vertex Shader
 		"struct vertex_output {" \
 			"float4 position : POSITION;" \
 		"};" \


 		//For Sending Output from Hull Constant Function
 		...
 		//For Setting Attributes For Patches And Control Points
 		"[domain(\"isoline\")]" \
	...
	ID3DBlob *pID3DBlob_HullShaderCode_RRJ = NULL;
	pID3DBlob_Error_RRJ = NULL;

	hr_RRJ = D3DCompile(
		hullShaderSourceCode_RRJ,
		lstrlenA(hullShaderSourceCode_RRJ) + 1,
		"HS",
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
	...
	if(FAILED(hr_RRJ)){
		if(pID3DBlob_Error_RRJ != NULL){
			...
			fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
			fprintf_s(gbFile_RRJ, "HULL SHADER ERROR: \n %s \n", (char*)pID3DBlob_Error_RRJ->GetBufferPointer());
			fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: Hull Shader Compilation Done!!\n");
 		fclose(gbFile_RRJ);
 	}	



 	hr_RRJ = gpID3D11Device_RRJ->CreateHullShader(
 		pID3DBlob_HullShaderCode_RRJ->GetBufferPointer(),
 		pID3DBlob_HullShaderCode_RRJ->GetBufferSize(),
 		NULL,
 		&gpID3D11HullShader_RRJ);


 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: Hull Shader Creation Failed!!\n");
 		fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: Hull Shader Created!!\n");
 		fclose(gbFile_RRJ);
 	}

 	gpID3D11DeviceContext_RRJ->HSSetShader(gpID3D11HullShader_RRJ, NULL, 0);
 	...
 	/********** DOMAIN SHADER **********/
 	const char *domainShaderSourceCode_RRJ = 
 		"cbuffer ConstantBuffer {" \
 			"float4x4 worldViewProjectionMatrix;" \
 		"};" \


 		//For Getting Input From HULL Constant Function
 		"struct hull_constant_output {" \
 			"float edges[2] : SV_TESSFACTOR;" \
 		"};" \


 		//For Getting Input From HULL Shader Main()
 		"struct hull_output { " \
 			"float4 position : POSITION;" \
 		"};" \


 		//For Domain Shader Output
	...
	ID3DBlob *pID3DBlob_DomainShaderCode_RRJ = NULL;
	pID3DBlob_Error_RRJ = NULL;


	hr_RRJ = D3DCompile(
		domainShaderSourceCode_RRJ,
		lstrlenA(domainShaderSourceCode_RRJ) + 1,
		"DS",
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
		"ds_5_0",
		0, 0,
		&pID3DBlob_DomainShaderCode_RRJ,
		&pID3DBlob_Error_RRJ);
	...
	if(FAILED(hr_RRJ)){
		if(pID3DBlob_Error_RRJ != NULL){
			...
			fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
			fprintf_s(gbFile_RRJ, "Domain SHADER ERROR: \n %s \n", (char*)pID3DBlob_Error_RRJ->GetBufferPointer());
			fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: Domain Shader Compilation Done!!\n");
 		fclose(gbFile_RRJ);
 	}	



 	hr_RRJ = gpID3D11Device_RRJ->CreateDomainShader(
 		pID3DBlob_DomainShaderCode_RRJ->GetBufferPointer(),
 		pID3DBlob_DomainShaderCode_RRJ->GetBufferSize(),
 		NULL,
 		&gpID3D11DomainShader_RRJ);


 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: Domain Shader Creation Failed!!\n");
 		fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: Domain Shader Created!!\n");
 		fclose(gbFile_RRJ);
 	}


 	gpID3D11DeviceContext_RRJ->DSSetShader(gpID3D11DomainShader_RRJ, NULL, 0);
 	...
 	inputElementDesc_RRJ.Format = DXGI_FORMAT_R32G32_FLOAT;
 	...
 	pID3DBlob_DomainShaderCode_RRJ->Release();
 	pID3DBlob_DomainShaderCode_RRJ = NULL;

 	pID3DBlob_HullShaderCode_RRJ->Release();
 	pID3DBlob_HullShaderCode_RRJ = NULL;
 	...
 	/********** Vertex Buffer Tri_Position **********/
 	D3D11_BUFFER_DESC bufferDesc_Tri_Pos_RRJ;
 	ZeroMemory((void*)&bufferDesc_Tri_Pos_RRJ, sizeof(D3D11_BUFFER_DESC));

 	bufferDesc_Tri_Pos_RRJ.Usage = D3D11_USAGE_DYNAMIC;
 	bufferDesc_Tri_Pos_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(line_Pos_RRJ);
 	bufferDesc_Tri_Pos_RRJ.BindFlags = D3D11_BIND_VERTEX_BUFFER;
 	bufferDesc_Tri_Pos_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_Tri_Pos_RRJ, NULL, &gpID3D11Buffer_VertexBuffer_Line_Position_RRJ);

 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: ID3D11Device::CreateBuffer() For Tri_Position Failed!!\n");
 		fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: ID3D11Device::CreateBuffer() For Tri_Position Done!!\n");
 		fclose(gbFile_RRJ);
 	}
 	...
 	gpID3D11DeviceContext_RRJ->Map(gpID3D11Buffer_VertexBuffer_Line_Position_RRJ, 0, 
 		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource_Tri_Position_RRJ);
 	memcpy(mappedSubresource_Tri_Position_RRJ.pData, line_Pos_RRJ, sizeof(line_Pos_RRJ));
 	gpID3D11DeviceContext_RRJ->Unmap(gpID3D11Buffer_VertexBuffer_Line_Position_RRJ, NULL);
 	...
 	bufferDesc_ConstantBuffer_RRJ.ByteWidth = sizeof(CBUFFER_HULL_SHADER);
 	...
 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_ConstantBuffer_RRJ, NULL,
 		&gpID3D11Buffer_ConstantBuffer_HullShader_RRJ);

 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: ID3D11Device::CreateBuffer() For Constant Buffer For Hull Shader Failed!!\n");
 		fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: ID3D11Device::CreateBuffer() For ConstantBuffer For Hull Shader Done!!\n");
 		fclose(gbFile_RRJ);
 	}
 	...
 	bufferDesc_ConstantBuffer_RRJ.Usage = D3D11_USAGE_DEFAULT;
 	bufferDesc_ConstantBuffer_RRJ.ByteWidth = sizeof(CBUFFER_DOMAIN_SHADER);
 	bufferDesc_ConstantBuffer_RRJ.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_ConstantBuffer_RRJ, NULL,
 		&gpID3D11Buffer_ConstantBuffer_DomainShader_RRJ);

 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: ID3D11Device::CreateBuffer() For Constant Buffer For Domain Shader Failed!!\n");
 		fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: ID3D11Device::CreateBuffer() For ConstantBuffer For Domain Shader Done!!\n");
 		fclose(gbFile_RRJ);
 	}
 	...
 	bufferDesc_ConstantBuffer_RRJ.Usage = D3D11_USAGE_DEFAULT;
 	bufferDesc_ConstantBuffer_RRJ.ByteWidth = sizeof(CBUFFER_PIXEL_SHADER);
 	bufferDesc_ConstantBuffer_RRJ.BindFlags = D3D11_BIND_CONSTANT_BUFFER;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_ConstantBuffer_RRJ, NULL,
 		&gpID3D11Buffer_ConstantBuffer_PixelShader_RRJ);

 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: ID3D11Device::CreateBuffer() For Constant Buffer For Pixel Shader Failed!!\n");
 		fclose(gbFile_RRJ);
 		...
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: ID3D11Device::CreateBuffer() For ConstantBuffer For Pixel Shader Done!!\n");
 		fclose(gbFile_RRJ);
 	}



 	gpID3D11DeviceContext_RRJ->HSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_HullShader_RRJ);
 	gpID3D11DeviceContext_RRJ->DSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_DomainShader_RRJ);
 	gpID3D11DeviceContext_RRJ->PSSetConstantBuffers(0, 1, &gpID3D11Buffer_ConstantBuffer_PixelShader_RRJ);
...
void uninitialize(void){
		...
		if(gpID3D11Buffer_ConstantBuffer_PixelShader_RRJ){
			gpID3D11Buffer_ConstantBuffer_PixelShader_RRJ->Release();
			gpID3D11Buffer_ConstantBuffer_PixelShader_RRJ = NULL;
		}


		if(gpID3D11Buffer_ConstantBuffer_DomainShader_RRJ){
			gpID3D11Buffer_ConstantBuffer_DomainShader_RRJ->Release();
			gpID3D11Buffer_ConstantBuffer_DomainShader_RRJ = NULL;
		}


		if(gpID3D11Buffer_ConstantBuffer_HullShader_RRJ){
			gpID3D11Buffer_ConstantBuffer_HullShader_RRJ->Release();
			gpID3D11Buffer_ConstantBuffer_HullShader_RRJ = NULL;
		}
		...
		if(gpID3D11Buffer_VertexBuffer_Line_Position_RRJ){
			gpID3D11Buffer_VertexBuffer_Line_Position_RRJ->Release();
			gpID3D11Buffer_VertexBuffer_Line_Position_RRJ = NULL;
		}
		...
		if(gpID3D11DomainShader_RRJ){
			gpID3D11DomainShader_RRJ->Release();
			gpID3D11DomainShader_RRJ = NULL;
		}

		if(gpID3D11HullShader_RRJ){
			gpID3D11HullShader_RRJ->Release();
			gpID3D11HullShader_RRJ = NULL;
		}
...
void display(void){
	...
	UINT offset = 0;
	gpID3D11DeviceContext_RRJ->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Line_Position_RRJ, &stride, &offset);


	gpID3D11DeviceContext_RRJ->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST);
	...
	//HULL Shader UNIFORM
	CBUFFER_HULL_SHADER constantBuffer_HullShader_RRJ;
	constantBuffer_HullShader_RRJ.Hull_Constant_Function_Params = XMVectorSet(1.0f, (float)guiNumberOfLineSegments_RRJ, 0.0f, 0.0f);
	gpID3D11DeviceContext_RRJ->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_HullShader_RRJ, 0,
		NULL, &constantBuffer_HullShader_RRJ, 0, 0);


	//DOMAIN Shader UNIFORM
	CBUFFER_DOMAIN_SHADER constantBuffer_DomainShader_RRJ;
	constantBuffer_DomainShader_RRJ.WorldViewProjectionMatrix = worldViewProjectionMatrix_RRJ;
	gpID3D11DeviceContext_RRJ->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_DomainShader_RRJ, 0,
		NULL, &constantBuffer_DomainShader_RRJ, 0, 0);

	
	//PIXEL Shader UNIFORM
	CBUFFER_PIXEL_SHADER constantBuffer_PixelShader_RRJ;
	constantBuffer_PixelShader_RRJ.LineColor = XMVectorSet(1.0f, 1.0f, 0.0f, 1.0f);
	gpID3D11DeviceContext_RRJ->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_PixelShader_RRJ, 0,
		NULL, &constantBuffer_PixelShader_RRJ, 0, 0);


	TCHAR str_RRJ[255];
	wsprintf(str_RRJ, TEXT("Rohit_R_Jadhav-D3D-31-Hull-Domain-Shader: [Segments : %d]"), guiNumberOfLineSegments_RRJ);
	SetWindowText(ghwnd_RRJ, str_RRJ);
