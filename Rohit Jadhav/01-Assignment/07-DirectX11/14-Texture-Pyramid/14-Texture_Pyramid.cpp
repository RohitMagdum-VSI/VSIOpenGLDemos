...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
 	...
 	/********** Vertex Buffer For Pyramid TEXCOORD **********/
 	D3D11_BUFFER_DESC bufferDesc_Pyramid_Texcoord_RRJ;
 	ZeroMemory((void*)&bufferDesc_Pyramid_Texcoord_RRJ, sizeof(D3D11_BUFFER_DESC));

 	bufferDesc_Pyramid_Texcoord_RRJ.Usage = D3D11_USAGE_DYNAMIC;
 	bufferDesc_Pyramid_Texcoord_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramid_Texcoord);
 	bufferDesc_Pyramid_Texcoord_RRJ.BindFlags = D3D11_BIND_VERTEX_BUFFER;
 	bufferDesc_Pyramid_Texcoord_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_Pyramid_Texcoord_RRJ, NULL,
...
void display(void){
	...
	gpID3D11DeviceContext_RRJ->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Pyramid_Texcoord_RRJ, &stride_Texcoord, &offset_Texcoord);
