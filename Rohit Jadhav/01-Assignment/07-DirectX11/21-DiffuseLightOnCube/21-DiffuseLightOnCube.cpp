...
ID3D11Buffer *gpID3D11Buffer_VertexBuffer_Cube_Normal_RRJ = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-21-DiffuseLightOnCube");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-21-DiffuseLightOnCube"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
		...
		"Vertex_Output main(float4 pos : POSITION, float4 normal : NORMAL){" \
			"Vertex_Output v;" \
			"if(keyPress == 1){ " \
				"float4 eyeCoord = mul(worldViewMatrix, pos);" \
				"float3 tNormal = normalize(mul((float3x3)worldViewMatrix, (float3)normal));" \
				"float3 source = (float3)normalize(lightPosition - eyeCoord);" \

				"v.diffuseLight = ld * kd * max(dot(source, tNormal), 0.0);" \
			"}" \
			"else{" \
				"v.diffuseLight = float4(1.0, 1.0, 1.0, 1.0);" \
			"}" \
			"v.position = mul(projectionMatrix, mul(worldViewMatrix, pos));" \
			"return(v);" \
 		"}";
 	...
 	/********** Vertex Buffer For Cube Normal **********/
 	D3D11_BUFFER_DESC bufferDesc_Cube_Normal_RRJ;
 	ZeroMemory((void*)&bufferDesc_Cube_Normal_RRJ, sizeof(D3D11_BUFFER_DESC));

 	bufferDesc_Cube_Normal_RRJ.Usage = D3D11_USAGE_DYNAMIC;
 	bufferDesc_Cube_Normal_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(cube_Normal);
 	bufferDesc_Cube_Normal_RRJ.BindFlags = D3D11_BIND_VERTEX_BUFFER;
 	bufferDesc_Cube_Normal_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_Cube_Normal_RRJ, NULL,
 		&gpID3D11Buffer_VertexBuffer_Cube_Normal_RRJ);

 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: ID3D11::CreateBuffer() for Cube_Color Failed!!\n");
 		fclose(gbFile_RRJ);
 		return(hr_RRJ);
 	}
 	else{
 	...
 	D3D11_MAPPED_SUBRESOURCE mappedSubresource_Cube_Normal_RRJ;
 	ZeroMemory((void*)&mappedSubresource_Cube_Normal_RRJ, sizeof(D3D11_MAPPED_SUBRESOURCE));

 	gpID3D11DeviceContext_RRJ->Map(gpID3D11Buffer_VertexBuffer_Cube_Normal_RRJ, 0, 
 		D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource_Cube_Normal_RRJ);

 	memcpy(mappedSubresource_Cube_Normal_RRJ.pData, cube_Normal, sizeof(cube_Normal));
 	gpID3D11DeviceContext_RRJ->Unmap(gpID3D11Buffer_VertexBuffer_Cube_Normal_RRJ, 0);
...
void uninitialize(void){
		...
		if(gpID3D11Buffer_VertexBuffer_Cube_Normal_RRJ){
			gpID3D11Buffer_VertexBuffer_Cube_Normal_RRJ->Release();
			gpID3D11Buffer_VertexBuffer_Cube_Normal_RRJ = NULL;
		}
...
void display(void){
	...
	UINT offset_Color = 0;
	gpID3D11DeviceContext_RRJ->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Cube_Normal_RRJ, &stride_Color, &offset_Color);
	...
	if(bLights_RRJ == true){
		...
		constantBuffer_RRJ.Ld = XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f); 
		constantBuffer_RRJ.Kd = XMVectorSet(0.5f, 0.5f, 0.5f, 1.0f);
		constantBuffer_RRJ.LightPosition = XMVectorSet(0.0f, 0.0f, -2.0f, 1.0f);
	}
	else
		constantBuffer_RRJ.LKeyPress = 0;

	constantBuffer_RRJ.WorldViewMatrix = worldViewMatrix_RRJ;
