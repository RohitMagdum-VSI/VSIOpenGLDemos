...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-08-Color-Rect-2");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-08-Color-Rect-2"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
 	...
 	/********** Vertex Buffer For Rect Color **********/
 	D3D11_BUFFER_DESC bufferDesc_Rect_Color_RRJ;
 	ZeroMemory((void*)&bufferDesc_Rect_Color_RRJ, sizeof(D3D11_BUFFER_DESC));

 	bufferDesc_Rect_Color_RRJ.Usage = D3D11_USAGE_DYNAMIC;
 	bufferDesc_Rect_Color_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(rect_Color);
 	bufferDesc_Rect_Color_RRJ.BindFlags = D3D11_BIND_VERTEX_BUFFER;
 	bufferDesc_Rect_Color_RRJ.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_Rect_Color_RRJ, NULL,
 		&gpID3D11Buffer_VertexBuffer_Rect_Color_RRJ);

 	if(FAILED(hr_RRJ)){
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "ERROR: ID3D11::CreateBuffer() for Rect_Color Failed!!\n");
 		fclose(gbFile_RRJ);
 		return(hr_RRJ);
 	}
 	else{
 		fopen_s(&gbFile_RRJ, gszLogFileName_RRJ, "a+");
 		fprintf_s(gbFile_RRJ, "SUCCESS: ID3D11::CreateBuffer() for Rect_Color Done!!\n");
 		fclose(gbFile_RRJ);	
 	}



 	/********** Memory Map I/O For Rect_Color **********/
...
void display(void){
	...
	UINT offset_Color = 0;
	gpID3D11DeviceContext_RRJ->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Rect_Color_RRJ, &stride_Color, &offset_Color);
