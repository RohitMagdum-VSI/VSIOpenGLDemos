...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow){
	...
	HWND hwnd_RRJ;
	MSG msg_RRJ;
	TCHAR szName_RRJ[] = TEXT("Rohit_R_Jadhav-D3D-11-Pyramid");
	...
	hwnd_RRJ = CreateWindowEx(WS_EX_APPWINDOW,
			szName_RRJ,
			TEXT("Rohit_R_Jadhav-D3D-11-Pyramid"),
			WS_OVERLAPPEDWINDOW,
			100, 100,
...
HRESULT initialize(void){
	...
	const char *vertexShaderSourceCode_RRJ = 
 	...
 	bufferDesc_Pyramid_Pos_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramid_Position);
 	...
 	hr_RRJ = gpID3D11Device_RRJ->CreateBuffer(&bufferDesc_Pyramid_Pos_RRJ, NULL, &gpID3D11Buffer_VertexBuffer_Pyramid_Position_RRJ);
 	...
 	bufferDesc_Pyramid_Color_RRJ.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramid_Color);
...
void display(void){
	...
	UINT offset_Color = 0;
	gpID3D11DeviceContext_RRJ->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Pyramid_Color_RRJ, &stride_Color, &offset_Color);
	...
	rotateMatrix_RRJ = XMMatrixRotationY(-angle_Pyramid);
	...
	angle_Pyramid = angle_Pyramid + 0.005f;
