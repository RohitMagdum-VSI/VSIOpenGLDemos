...
#define WINDOW_NAME		L"D3D - Single Light On Sphere Per Vertex And Fragment"
...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode_PerVertex =
		...
		*/
		...
		"float3 light_direction= normalize((float3)vecLightPosition - eyeCoordinates.xyz);"								\
		...
		"float4 specular = vecLS * vecKS * pow(max(dot(reflection_vector, viewer_vector), 0.0), fMaterialShininess);"								\
