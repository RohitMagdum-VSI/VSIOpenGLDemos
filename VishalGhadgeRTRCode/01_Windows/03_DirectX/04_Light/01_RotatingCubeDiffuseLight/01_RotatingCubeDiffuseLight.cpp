...
#define WINDOW_NAME		L"D3D-Rotating Cube With Diffuse Light"
...
ID3D11Buffer *g_pID3D11Buffer_VertexBufferCubeNormal = NULL;	//	vbo_position in openGL
...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode =
		...
		*/
		...
		"vertex_output output;"								\
			...
			"float3 srcVec= normalize((float3)(vecLightPosition - eyeCoordinates));"								\
			"output.DiffuseLight = vecLD * vecKD * max(dot(srcVec,tnorm), 0.0);"								\
			"}"								\
			"else"								\
			"{"								\
	...
	ZeroMemory(&BufferDesc, sizeof(BufferDesc));
	BufferDesc.Usage = D3D11_USAGE_DYNAMIC;	//	DirectX prefer dynamic drawing.
	BufferDesc.ByteWidth = sizeof(float)* ARRAYSIZE(farrCubeNormal);
	BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferCubeNormal
		);
	if (FAILED(hr))
	{
		log_write("CreateBuffer() failed for vertex buffer.\n");
	...
	ZeroMemory(&MappedSubresource, sizeof(MappedSubresource));
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferCubeNormal,	//	ID3D11Buffer Inherit from ID3D11Resource 
		0,	//Index number of subresource i.e like 0 = position, 1 = color
		D3D11_MAP_WRITE_DISCARD,	//	specifies the CPU's read and write permissions for a resource.
		0,	//	Flag that specifies what the CPU does when the GPU is busy. This flag is optional.Cpu will wait.
		&MappedSubresource
	...
	CopyMemory(MappedSubresource.pData, farrCubeNormal, sizeof(farrCubeNormal));

	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferCubeNormal, 0);
...
VOID display()
{
	...
	if (true == g_bLight)
	{
		constantBuffer.vecLD = XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f);
		constantBuffer.vecKD = XMVectorSet(0.5f, 0.5f, 0.5f, 1.0f);
		constantBuffer.vecLightPosition = XMVectorSet(0.0f, 0.0f, -2.0f, 1.0f);
...
VOID uninitialize()
{
	...
	if (g_pID3D11Buffer_VertexBufferCubeNormal)
	{
		g_pID3D11Buffer_VertexBufferCubeNormal->Release();
		g_pID3D11Buffer_VertexBufferCubeNormal = NULL;
	}
