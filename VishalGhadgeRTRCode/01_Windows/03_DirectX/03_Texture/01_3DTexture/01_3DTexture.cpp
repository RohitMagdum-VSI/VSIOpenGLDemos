...
ID3D11Buffer *g_pID3D11Buffer_VertexBufferPyramidTexture = NULL;	//	vbo_position in openGL
...
ID3D11ShaderResourceView *g_pID3D11ShaderResourceView_TexturePyramid = NULL;
ID3D11SamplerState *g_pID3D11SamplerState_TexturePyramid = NULL;
...
HRESULT initialize()
{
	...
	/////////////////////////////////////////////////////////////////////////////////////////////
	//+	Create texture buffer Triangle

	//	Winding - Clock wise
	//	Left hand rule
	float farrPyramidTexture[] =
	{
		0.5f, 1.0f,
		1.0f, 0.0f,
		0.0f, 0.0f,
	...
	ZeroMemory(&BufferDesc, sizeof(BufferDesc));
	BufferDesc.Usage = D3D11_USAGE_DYNAMIC;	//	DirectX prefer dynamic drawing.
	BufferDesc.ByteWidth = sizeof(float)* ARRAYSIZE(farrPyramidTexture);
	BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferPyramidTexture
		);
	if (FAILED(hr))
	{
		log_write("CreateBuffer() failed for vertex buffer.\n");
	...
	ZeroMemory(&MappedSubresource, sizeof(MappedSubresource));
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferPyramidTexture,	//	ID3D11Buffer Inherit from ID3D11Resource 
		0,	//Index number of subresource i.e like 0 = position, 1 = color
		D3D11_MAP_WRITE_DISCARD,	//	specifies the CPU's read and write permissions for a resource.
		0,	//	Flag that specifies what the CPU does when the GPU is busy. This flag is optional.Cpu will wait.
		&MappedSubresource
	...
	CopyMemory(MappedSubresource.pData, farrPyramidTexture, sizeof(farrPyramidTexture));

	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferPyramidTexture, 0);
	...
	/////////////////////////////////////////////////////////////////////////////////////////////
	//+	Pyramid : Create texture resource and texture view

	hr = LoadD3DTexture(L"Stone.bmp", &g_pID3D11ShaderResourceView_TexturePyramid);
	if (FAILED(hr))
	{
		log_write("LoadD3DTexture() failed.");
		return hr;
	}
	...
	hr = g_pID3D11Device->CreateSamplerState(&SamplerDesc, &g_pID3D11SamplerState_TexturePyramid);
	if (FAILED(hr))
	{
		log_write("CreateSamplerState(Pyramid) failed.");
		return hr;
	}
	...
	hr = LoadD3DTexture(L"Kundali.bmp", &g_pID3D11ShaderResourceView_TextureCube);
	if (FAILED(hr))
	{
...
VOID display()
{
	...
	g_pID3D11DeviceContext->PSSetShaderResources(0, 1, &g_pID3D11ShaderResourceView_TexturePyramid);
	g_pID3D11DeviceContext->PSSetSamplers(0, 1, &g_pID3D11SamplerState_TexturePyramid);
...
VOID uninitialize()
{
	...
	if (g_pID3D11SamplerState_TexturePyramid)
	{
		g_pID3D11SamplerState_TexturePyramid->Release();
		g_pID3D11SamplerState_TexturePyramid = NULL;
	}
	...
	if (g_pID3D11ShaderResourceView_TexturePyramid)
	{
		g_pID3D11ShaderResourceView_TexturePyramid->Release();
		g_pID3D11ShaderResourceView_TexturePyramid = NULL;
	}
	...
	if (g_pID3D11Buffer_VertexBufferPyramidTexture)
	{
		g_pID3D11Buffer_VertexBufferPyramidTexture->Release();
		g_pID3D11Buffer_VertexBufferPyramidTexture = NULL;
	}
