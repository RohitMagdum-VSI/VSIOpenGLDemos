...
BYTE g_arrCheckerImage[IMAGE_HEIGHT][IMAGE_WIDTH][4];
int g_iDigit = 0x31;

#define WINDOW_NAME		L"D3D-CheckerBoard"
...
HRESULT initialize()
{
	...
	hr = LoadD3DProceduralTexture(&g_pID3D11ShaderResourceView_TextureCube);
	if (FAILED(hr))
	{
...
HRESULT LoadD3DProceduralTexture(ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	...
	dwBufferSize = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER) + (3 * w * h);
	int filesize = 54 + 3 * w * h;  //w is your image width, h is image height, both int

	buffer = (unsigned char *)malloc(dwBufferSize);
	memset(buffer, 0, dwBufferSize);

	pBitmapFileHeader = (BITMAPFILEHEADER*)buffer;
	pBitmapFileHeader->bfType = 0x4d42;
	pBitmapFileHeader->bfSize = dwBufferSize;
	pBitmapFileHeader->bfOffBits = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER);

	pBitmapInfoHeader = (BITMAPINFOHEADER*)(buffer + sizeof(BITMAPFILEHEADER));
	pBitmapInfoHeader->biSize = sizeof(BITMAPINFOHEADER);
	pBitmapInfoHeader->biWidth = IMAGE_WIDTH;
	pBitmapInfoHeader->biHeight = IMAGE_HEIGHT;
	...
	pBitmapInfoHeader->biXPelsPerMeter = 3780;
	pBitmapInfoHeader->biYPelsPerMeter = 3780;

	img = (unsigned char *)buffer + pBitmapFileHeader->bfOffBits;
	...
	hr = DirectX::CreateWICTextureFromMemory(g_pID3D11Device, g_pID3D11DeviceContext, buffer, (size_t)(dwBufferSize), NULL, ppID3D11ShaderResourceView);// , NULL);
	if (FAILED(hr))
	{
		log_write("CreateWICTextureFromMemory() failed.");
		return(hr);
	}
...
VOID display()
{
	...
	float farrCubeVertices[18];

	farrCubeVertices[0] = -2.0f;
	farrCubeVertices[1] = 1.0f;
	farrCubeVertices[2] = 0.0f;
	farrCubeVertices[3] = 0.0f;
	farrCubeVertices[4] = 1.0f;
	farrCubeVertices[5] = 0.0f;
	farrCubeVertices[6] = -2.0f;
	farrCubeVertices[7] = -1.0f;
	farrCubeVertices[8] = 0.0f;
	farrCubeVertices[9] = -2.0f;
	farrCubeVertices[10] = -1.0f;
	farrCubeVertices[11] = 0.0f;
	farrCubeVertices[12] = 0.0f;
	farrCubeVertices[13] = 1.0f;
	farrCubeVertices[14] = 0.0f;
	farrCubeVertices[15] = 0.0f;
	farrCubeVertices[16] = -1.0f;
	farrCubeVertices[17] = 0.0f;
	...
	/////////////////////////////////////////////////////////////////////////////////////////
	//+ Draw Square
	farrCubeVertices[0] = 1.0f;
	farrCubeVertices[1] = 1.0f;
	farrCubeVertices[2] = 0.0f;
	farrCubeVertices[3] = 2.41421f;
	farrCubeVertices[4] = 1.0f;
	farrCubeVertices[5] = 1.41421f;
	farrCubeVertices[6] = 1.0f;
	farrCubeVertices[7] = -1.0f;
	farrCubeVertices[8] = 0.0f;
	farrCubeVertices[9] = 1.0f;
	farrCubeVertices[10] = -1.0f;
	farrCubeVertices[11] = 0.0f;
	farrCubeVertices[12] = 2.41421f;
	farrCubeVertices[13] = 1.0f;
	farrCubeVertices[14] = 1.41421f;
	farrCubeVertices[15] = 2.41421f;
	farrCubeVertices[16] = -1.0f;
	farrCubeVertices[17] = 1.41421f;
