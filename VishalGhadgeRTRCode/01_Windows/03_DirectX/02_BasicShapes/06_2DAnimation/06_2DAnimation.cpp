...
#define WINDOW_NAME		L"D3D-Color Triangle Square"
...
ID3D11Buffer *g_pID3D11Buffer_VertexBufferColorTriangle = NULL;	//	vbo_position in openGL
ID3D11Buffer *g_pID3D11Buffer_VertexBufferPositionSquare = NULL;	//	vbo_position in openGL
ID3D11Buffer *g_pID3D11Buffer_VertexBufferColorSquare = NULL;	//	vbo_position in openGL
...
HRESULT initialize()
{
	...
	ZeroMemory(&BufferDesc, sizeof(BufferDesc));
	BufferDesc.Usage = D3D11_USAGE_DYNAMIC;	//	DirectX prefer dynamic drawing.
	BufferDesc.ByteWidth = sizeof(float)* ARRAYSIZE(farrTriangleColor);
	BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferColorTriangle
		);
	if (FAILED(hr))
	{
		log_write("CreateBuffer() failed for vertex buffer.\n");
	...
	ZeroMemory(&MappedSubresource, sizeof(MappedSubresource));
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferColorTriangle,	//	ID3D11Buffer Inherit from ID3D11Resource 
		0,	//Index number of subresource i.e like 0 = position, 1 = color
		D3D11_MAP_WRITE_DISCARD,	//	specifies the CPU's read and write permissions for a resource.
		0,	//	Flag that specifies what the CPU does when the GPU is busy. This flag is optional.Cpu will wait.
		&MappedSubresource
	...
	CopyMemory(MappedSubresource.pData, farrTriangleColor, sizeof(farrTriangleColor));

	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferColorTriangle, 0);
	...
	ZeroMemory(&BufferDesc, sizeof(BufferDesc));
	BufferDesc.Usage = D3D11_USAGE_DYNAMIC;	//	DirectX prefer dynamic drawing.
	BufferDesc.ByteWidth = sizeof(float)* ARRAYSIZE(farrSquareColor);
	BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferColorSquare
		);
	if (FAILED(hr))
	{
		log_write("CreateBuffer() failed for vertex buffer.\n");
	...
	ZeroMemory(&MappedSubresource, sizeof(MappedSubresource));
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferColorSquare,	//	ID3D11Buffer Inherit from ID3D11Resource 
		0,	//Index number of subresource i.e like 0 = position, 1 = color
		D3D11_MAP_WRITE_DISCARD,	//	specifies the CPU's read and write permissions for a resource.
		0,	//	Flag that specifies what the CPU does when the GPU is busy. This flag is optional.Cpu will wait.
		&MappedSubresource
	...
	CopyMemory(MappedSubresource.pData, farrSquareColor, sizeof(farrSquareColor));

	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferColorSquare, 0);
...
VOID display()
{
	...
	worldMatrix = XMMatrixRotationX(XMConvertToRadians(g_fAngleRect));
...
VOID uninitialize()
{
	...
	if (g_pID3D11Buffer_VertexBufferColorTriangle)
	{
		g_pID3D11Buffer_VertexBufferColorTriangle->Release();
		g_pID3D11Buffer_VertexBufferColorTriangle = NULL;
	}
	...
	if (g_pID3D11Buffer_VertexBufferColorSquare)
	{
		g_pID3D11Buffer_VertexBufferColorSquare->Release();
		g_pID3D11Buffer_VertexBufferColorSquare = NULL;
	}
