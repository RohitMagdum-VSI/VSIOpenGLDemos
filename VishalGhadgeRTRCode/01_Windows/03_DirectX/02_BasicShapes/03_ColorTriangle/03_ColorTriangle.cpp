...
#define WINDOW_NAME		L"D3D-Triangle Perspective"
...
ID3D11Buffer *g_pID3D11Buffer_VertexBufferPosition = NULL;	//	vbo_position in openGL
...
HRESULT initialize()
{
	...
	hr = g_pID3D11Device->CreateBuffer(
		...
		&g_pID3D11Buffer_VertexBufferPosition
		);
	if (FAILED(hr))
	{
	...
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferPosition,	//	ID3D11Buffer Inherit from ID3D11Resource 
	...
	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferPosition, 0);
...
VOID uninitialize()
{
	...
	if (g_pID3D11Buffer_VertexBufferPosition)
	{
		g_pID3D11Buffer_VertexBufferPosition->Release();
		g_pID3D11Buffer_VertexBufferPosition = NULL;
	}
