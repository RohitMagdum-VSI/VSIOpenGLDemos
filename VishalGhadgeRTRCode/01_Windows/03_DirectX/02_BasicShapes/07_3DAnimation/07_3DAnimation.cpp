...
#define WINDOW_NAME		L"D3D-3D Animation"
...
ID3D11Buffer *g_pID3D11Buffer_VertexBufferPositionPyramid = NULL;	//	vbo_position in openGL
ID3D11Buffer *g_pID3D11Buffer_VertexBufferColorPyramid = NULL;	//	vbo_position in openGL
ID3D11Buffer *g_pID3D11Buffer_VertexBufferPositionCube = NULL;	//	vbo_position in openGL
ID3D11Buffer *g_pID3D11Buffer_VertexBufferColorCube = NULL;	//	vbo_position in openGL
...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode =
		...
		*/
		...
		"vertex_output main(float4 pos:POSITION, float4 col:COLOR)	"/* : SV_POSITION"*/				\
		"{"								\
	...
	InputElementDesc[RTR_INPUT_SLOT_COLOR].SemanticName = "COLOR";
	InputElementDesc[RTR_INPUT_SLOT_COLOR].SemanticIndex = 0;	//	A semantic index is only needed in a case where there is more than one element with the same semantic.we will use in case of structure.
	InputElementDesc[RTR_INPUT_SLOT_COLOR].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	InputElementDesc[RTR_INPUT_SLOT_COLOR].InputSlot = RTR_INPUT_SLOT_COLOR;//	An integer value that identifies the input-assembler (see input slot). Valid values are between 0 and 15, defined in D3D11.h.
	InputElementDesc[RTR_INPUT_SLOT_COLOR].AlignedByteOffset = 0;
	InputElementDesc[RTR_INPUT_SLOT_COLOR].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	InputElementDesc[RTR_INPUT_SLOT_COLOR].InstanceDataStepRate = 0;
	...
	BufferDesc.ByteWidth = sizeof(float)* ARRAYSIZE(farrPyramidVertices);
	BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferPositionPyramid
		);
	if (FAILED(hr))
	{
	...
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferPositionPyramid,	//	ID3D11Buffer Inherit from ID3D11Resource 
	...
	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferPositionPyramid, 0);
	...
	ZeroMemory(&BufferDesc, sizeof(BufferDesc));
	BufferDesc.Usage = D3D11_USAGE_DYNAMIC;	//	DirectX prefer dynamic drawing.
	BufferDesc.ByteWidth = sizeof(float)* ARRAYSIZE(farrPyramidColor);
	BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferColorPyramid
		);
	if (FAILED(hr))
	{
		log_write("CreateBuffer() failed for vertex buffer.\n");
	...
	ZeroMemory(&MappedSubresource, sizeof(MappedSubresource));
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferColorPyramid,	//	ID3D11Buffer Inherit from ID3D11Resource 
		0,	//Index number of subresource i.e like 0 = position, 1 = color
		D3D11_MAP_WRITE_DISCARD,	//	specifies the CPU's read and write permissions for a resource.
		0,	//	Flag that specifies what the CPU does when the GPU is busy. This flag is optional.Cpu will wait.
		&MappedSubresource
	...
	CopyMemory(MappedSubresource.pData, farrPyramidColor, sizeof(farrPyramidColor));

	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferColorPyramid, 0);
	...
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferPositionCube
		);
	if (FAILED(hr))
	{
		log_write("CreateBuffer() failed for vertex buffer.\n");
	...
	ZeroMemory(&MappedSubresource, sizeof(MappedSubresource));
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferPositionCube,	//	ID3D11Buffer Inherit from ID3D11Resource 
		0,	//Index number of subresource i.e like 0 = position, 1 = color
		D3D11_MAP_WRITE_DISCARD,	//	specifies the CPU's read and write permissions for a resource.
		0,	//	Flag that specifies what the CPU does when the GPU is busy. This flag is optional.Cpu will wait.
		&MappedSubresource
	...
	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferPositionCube, 0);
	...
	ZeroMemory(&BufferDesc, sizeof(BufferDesc));
	BufferDesc.Usage = D3D11_USAGE_DYNAMIC;	//	DirectX prefer dynamic drawing.
	BufferDesc.ByteWidth = sizeof(float)* ARRAYSIZE(farrCubeColor);
	BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferColorCube
		);
	if (FAILED(hr))
	{
		log_write("CreateBuffer() failed for vertex buffer.\n");
	...
	ZeroMemory(&MappedSubresource, sizeof(MappedSubresource));
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferColorCube,	//	ID3D11Buffer Inherit from ID3D11Resource 
		0,	//Index number of subresource i.e like 0 = position, 1 = color
		D3D11_MAP_WRITE_DISCARD,	//	specifies the CPU's read and write permissions for a resource.
		0,	//	Flag that specifies what the CPU does when the GPU is busy. This flag is optional.Cpu will wait.
		&MappedSubresource
	...
	CopyMemory(MappedSubresource.pData, farrCubeColor, sizeof(farrCubeColor));

	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferColorCube, 0);
...
VOID display()
{
	...
	worldMatrix = worldMatrix * XMMatrixTranslation(-2.5f, 0.0f, 6.0f);
	...
	rotationMatrixR1 = XMMatrixRotationX(XMConvertToRadians(g_fAngleRect));
	rotationMatrixR2 = XMMatrixRotationY(XMConvertToRadians(g_fAngleRect));
	rotationMatrixR3 = XMMatrixRotationZ(XMConvertToRadians(g_fAngleRect));
	rotationMatrix = rotationMatrixR1 * rotationMatrixR2 * rotationMatrixR3;
	...
	g_pID3D11DeviceContext->Draw(6, 6);
	g_pID3D11DeviceContext->Draw(6, 12);
	g_pID3D11DeviceContext->Draw(6, 18);
	g_pID3D11DeviceContext->Draw(6, 24);
	g_pID3D11DeviceContext->Draw(6, 30);
...
VOID uninitialize()
{
	...
	if (g_pID3D11Buffer_VertexBufferPositionPyramid)
	{
		g_pID3D11Buffer_VertexBufferPositionPyramid->Release();
		g_pID3D11Buffer_VertexBufferPositionPyramid = NULL;
	}

	if (g_pID3D11Buffer_VertexBufferColorPyramid)
	{
		g_pID3D11Buffer_VertexBufferColorPyramid->Release();
		g_pID3D11Buffer_VertexBufferColorPyramid = NULL;
	}

	if (g_pID3D11Buffer_VertexBufferPositionCube)
	{
		g_pID3D11Buffer_VertexBufferPositionCube->Release();
		g_pID3D11Buffer_VertexBufferPositionCube = NULL;
	}

	if (g_pID3D11Buffer_VertexBufferColorCube)
	{
		g_pID3D11Buffer_VertexBufferColorCube->Release();
		g_pID3D11Buffer_VertexBufferColorCube = NULL;
	}
