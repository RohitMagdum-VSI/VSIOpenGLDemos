...
#define WINDOW_NAME		L"D3D-Triangle Ortho"
...
HRESULT resize(int iWidth, int iHeight)
{
	...
	if (iWidth <= iHeight)
	{
		g_OrthographicProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f, 100.0f, -100.0f * ((float)iHeight / (float)iWidth), 100.0f * ((float)iHeight / (float)iWidth), -100.0f, 100.0f);
	}
	else
	{
		g_OrthographicProjectionMatrix = XMMatrixOrthographicOffCenterLH(-100.0f * ((float)iWidth / (float)iHeight), 100.0f * ((float)iWidth / (float)iHeight), -100.0f, 100.0f, -100.0f, 100.0f);
	}
	return hr;
}
