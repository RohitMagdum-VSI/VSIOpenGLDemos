...
#define WINDOW_NAME		L"D3D-Interleaved 2D Array"
...
float g_farrLightAmbient[] = { 0.25f, 0.25f, 0.25f, 1.0f };
...
float g_fMaterialShininess = 128.0f;
...
ID3D11Buffer *g_pID3D11Buffer_VertexBufferCube = NULL;	//	vbo_position in openGL
...
HRESULT initialize()
{
	...
	const char *vertexShaderSourceCode =
		...
		*/
		...
		"vertex_output main(float4 pos:POSITION, float4 color:COLOR, float4 norm:NORMAL, float2 texcoord:TEXCOORD)"/* : SV_POSITION"*/				\
		"{"								\
	...
	const char *pixelShaderSourceCode =
		...
		"}"								\
		"output_color = myTexture2D.Sample(mySamplerState, input.texcoord) * input.color * output_color;"		\
		"return output_color;"		\
		"}";
	...
	{
		{ "POSITION" , 0, DXGI_FORMAT_R32G32B32_FLOAT, RTR_INPUT_SLOT_POSITION, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "COLOR" , 0, DXGI_FORMAT_R32G32B32_FLOAT, RTR_INPUT_SLOT_COLOR, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "NORMAL" , 0, DXGI_FORMAT_R32G32B32_FLOAT, RTR_INPUT_SLOT_NORMAL, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD" , 0, DXGI_FORMAT_R32G32_FLOAT, RTR_INPUT_SLOT_TEXTURE, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 },
	};
	...
	const CUBE_VERTEX glfarrCubeVCNT[36] =
	{
		// SIDE 1 ( TOP )
		// triangle 1
	...
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferCube,	//	ID3D11Buffer Inherit from ID3D11Resource 
	...
	CopyMemory(MappedSubresource.pData, glfarrCubeVCNT, sizeof(glfarrCubeVCNT));

	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferCube, 0);
	...
	hr = LoadD3DTexture(L"marble.bmp", &g_pID3D11ShaderResourceView_TextureCube);
	if (FAILED(hr))
	{
		log_write("LoadD3DTexture(marble) failed.");
		return hr;
	}
...
VOID display()
{
	...
	/////////////////////////////////////////////////////////////////////////////////////////
	//+ Draw Square
	uiStride = sizeof(float) * 11;
	uiOffset = 0 * sizeof(float);
	...
	uiStride = sizeof(float) * 11;
	uiOffset = 9 * sizeof(float);
	g_pID3D11DeviceContext->IASetVertexBuffers(
		RTR_INPUT_SLOT_TEXTURE,	//	0 - Position, 1 - color
		1,
		&g_pID3D11Buffer_VertexBufferCube,
		&uiStride,
		&uiOffset
...
VOID uninitialize()
{
	...
	if (g_pID3D11Buffer_VertexBufferCube)
	{
		g_pID3D11Buffer_VertexBufferCube->Release();
		g_pID3D11Buffer_VertexBufferCube = NULL;
	}
