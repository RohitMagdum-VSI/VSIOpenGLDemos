...
#define WINDOW_NAME		L"D3D-Geometry Shader Color"
...
ID3D11GeometryShader *g_pID3D11GeometryShader = NULL;		//	same as fragment shader in OpenGL
...
ID3D11Buffer *g_pID3D11Buffer_VertexBufferColor = NULL;	//	vbo_position in openGL
...
HRESULT initialize()
{
	...
	const char *geometryShaderSourceCode =
		...
		*/
		...
		"{"								\
		"output.position = mul(worldViewProjectionMatrix, (input[iVertex].position + float4(0.0f,1.0f,0.0f,0.0f)));"								\
		"triangleStream.Append(output)"								\
		"output.position = mul(worldViewProjectionMatrix, (input[iVertex].position + float4(1.0f,-1.0f,0.0f,0.0f)));"								\
		"triangleStream.Append(output)"								\
		"output.position = mul(worldViewProjectionMatrix, (input[iVertex].position + float4(-1.0f,-1.0f,0.0f,0.0f)));"								\
		"triangleStream.Append(output)"								\
	...
	ID3DBlob *pID3DBlob_GeometryShaderByteCode = NULL;
	pID3DBlob_Error = NULL;

	hr = D3DCompile(
		geometryShaderSourceCode,
		lstrlenA(geometryShaderSourceCode) + 1,	//	+ 1 for null character.
		"GS",
		NULL,	//	macros in shader program, currently we are not using any macro in program.
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",	//	Entry point function in shader
		...
		0,	//	Compiler constants
		0, //	Effect constant
		&pID3DBlob_GeometryShaderByteCode,
		&pID3DBlob_Error
	);
	if (FAILED(hr))
	{
		fopen_s(&g_pFile, LOG_FILE, "a+");
		fprintf_s(g_pFile, "D3DCompile failed for Geometry Shader.");
		if (NULL != pID3DBlob_Error)
		{
			fprintf_s(g_pFile, "Error %s.\n", (char*)pID3DBlob_Error->GetBufferPointer());
			pID3DBlob_Error->Release();
	...
	hr = g_pID3D11Device->CreateGeometryShader(
		pID3DBlob_GeometryShaderByteCode->GetBufferPointer(),
		pID3DBlob_GeometryShaderByteCode->GetBufferSize(),
		NULL,	// To pass the data across the shaders
		&g_pID3D11GeometryShader
	);
	if (FAILED(hr))
	{
		log_write("CreateVertexShader(Geometry) failed.\n");
		//	Cleanup local only, global will be clean in uninitialize().
		pID3DBlob_GeometryShaderByteCode->Release();
		pID3DBlob_GeometryShaderByteCode = NULL;
		return hr;
	}

	g_pID3D11DeviceContext->GSSetShader(g_pID3D11GeometryShader, 0, 0);
	...
	ZeroMemory(&BufferDesc, sizeof(BufferDesc));
	BufferDesc.Usage = D3D11_USAGE_DYNAMIC;	//	DirectX prefer dynamic drawing.
	BufferDesc.ByteWidth = sizeof(float)* ARRAYSIZE(farrColor);
	BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	hr = g_pID3D11Device->CreateBuffer(
		&BufferDesc,
		NULL,	//	Pass data dynamically.
		&g_pID3D11Buffer_VertexBufferColor
	);
	if (FAILED(hr))
	{
		log_write("CreateBuffer() failed for vertex buffer.\n");
	...
	ZeroMemory(&MappedSubresource, sizeof(MappedSubresource));
	g_pID3D11DeviceContext->Map(
		g_pID3D11Buffer_VertexBufferColor,	//	ID3D11Buffer Inherit from ID3D11Resource 
		0,	//Index number of subresource i.e like 0 = position, 1 = color
		D3D11_MAP_WRITE_DISCARD,	//	specifies the CPU's read and write permissions for a resource.
		0,	//	Flag that specifies what the CPU does when the GPU is busy. This flag is optional.Cpu will wait.
		&MappedSubresource
	...
	CopyMemory(MappedSubresource.pData, farrColor, sizeof(farrColor));

	g_pID3D11DeviceContext->Unmap(g_pID3D11Buffer_VertexBufferColor, 0);
	...
	g_pID3D11DeviceContext->GSSetConstantBuffers(
		0,//	Index into the device's zero-based array to begin setting constant buffers to (ranges from 0 to D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - 1).
		1,	//	Number of buffers.
		&g_pID3D11Buffer_ConstantBuffer
	);
...
VOID uninitialize()
{
	...
	if (g_pID3D11Buffer_VertexBufferColor)
	{
		g_pID3D11Buffer_VertexBufferColor->Release();
		g_pID3D11Buffer_VertexBufferColor = NULL;
	}
	...
	if (g_pID3D11GeometryShader)
	{
		g_pID3D11GeometryShader->Release();
		g_pID3D11GeometryShader = NULL;
	}
