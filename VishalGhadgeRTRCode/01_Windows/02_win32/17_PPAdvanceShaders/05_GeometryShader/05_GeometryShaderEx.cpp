...
#define	CLASS_NAME		TEXT("PP : Geometry Shader")
...
GLuint g_gluiShaderObjectGeometry;
...
VOID initialize()
{
	...
	//	Create shader.
	g_gluiShaderObjectGeometry = glCreateShader(GL_GEOMETRY_SHADER);

	//	Provide source code.
	const GLchar *szGeometryShaderSourceCode =
		"#version 430 core"							\
		"\n"										\
		"layout(triangles) in;"						\
		...
		"{"											\
			...
			"{"											\
				"gcolor = out_color[iVertex];"	\
				"gl_Position = u_mvp_matrix * (gl_in[iVertex].gl_Position + vec4(0.0,1.0,0.0,0.0));"	\
				"EmitVertex();"	\
				"gl_Position = u_mvp_matrix * (gl_in[iVertex].gl_Position + vec4(-1.0,-1.0,0.0,0.0));"	\
				"EmitVertex();"	\
				"gl_Position = u_mvp_matrix * (gl_in[iVertex].gl_Position + vec4(1.0,-1.0,0.0,0.0));"	\
				"EmitVertex();"	\
				"EndPrimitive();"	\
	...
	glShaderSource(g_gluiShaderObjectGeometry, 1, &szGeometryShaderSourceCode, NULL);

	//	Compile shader.
	glCompileShader(g_gluiShaderObjectGeometry);

	//GLint gliCompileStatus;
	//GLint gliInfoLogLength;
	//char *pszInfoLog = NULL;
	//GLsizei glsiWritten;
	glGetShaderiv(g_gluiShaderObjectGeometry, GL_COMPILE_STATUS, &gliCompileStatus);
	if (GL_FALSE == gliCompileStatus)
	{
		glGetShaderiv(g_gluiShaderObjectGeometry, GL_INFO_LOG_LENGTH, &gliInfoLogLength);
		if (gliInfoLogLength <= 0)
		{
			fprintf(g_fpLogFile, "GL_INFO_LOG_LENGTH is less than 0.");
			uninitialize();
		...
		glGetShaderInfoLog(g_gluiShaderObjectGeometry, gliInfoLogLength, &glsiWritten, pszInfoLog);
	...
	glAttachShader(g_gluiShaderObjectProgram, g_gluiShaderObjectGeometry);
...
VOID uninitialize()
{
	...
	if (g_gluiShaderObjectGeometry)
	{
		glDetachShader(g_gluiShaderObjectProgram, g_gluiShaderObjectGeometry);
		glDeleteShader(g_gluiShaderObjectGeometry);
		g_gluiShaderObjectGeometry = 0;
	}
