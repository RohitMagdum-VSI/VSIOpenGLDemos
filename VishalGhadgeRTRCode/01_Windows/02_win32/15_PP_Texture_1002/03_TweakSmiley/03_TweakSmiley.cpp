...
#define	CLASS_NAME		TEXT("PP : Tweak Smiley")
...
GLuint	g_gluiTextureWhite;

GLubyte g_arrWhiteImage[IMAGE_HEIGHT][IMAGE_WIDTH][4];
...
VOID initialize()
{
	...
	glBufferData(GL_ARRAY_BUFFER, (2 * 4 * sizeof(GLfloat)), 0, GL_DYNAMIC_DRAW);	//	2(s,t text cord) * 4(text cord for 4 vertices) * sizeof(float) i.e)glfarrQuadTextCoord[8]
	...
	LoadGLTextures(&g_gluiTextureSmiley, MAKEINTRESOURCE(IDBITMAP_SMILEY));
	LoadGLWhiteTextures();
...
BOOLEAN LoadGLWhiteTextures()
{
	...
	glGenTextures(1, &g_gluiTextureWhite);

	glBindTexture(GL_TEXTURE_2D, g_gluiTextureWhite);	//	Bind texture.
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, IMAGE_WIDTH, IMAGE_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, g_arrWhiteImage);
...
void MakeCheckImage()
{
	...
	for (i = 0; i < IMAGE_HEIGHT; i++)
	{
		for (j = 0; j < IMAGE_WIDTH; j++)
		{
			...
			c = 1 * 255;
			g_arrWhiteImage[i][j][0] = (GLubyte)c;
			g_arrWhiteImage[i][j][1] = (GLubyte)c;
			g_arrWhiteImage[i][j][2] = (GLubyte)c;
			g_arrWhiteImage[i][j][3] = (GLubyte)c;
		}
	}
}
...
VOID display()
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, g_gluiVBOTexture);
	glBufferData(GL_ARRAY_BUFFER, sizeof(arrQuadTextureCoord), arrQuadTextureCoord, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	glActiveTexture(GL_TEXTURE0);
	if (0 == g_iDigit)
	{
		glBindTexture(GL_TEXTURE_2D, g_gluiTextureWhite);
	}
	else
	{
...
VOID uninitialize()
{
	...
	if (g_gluiTextureWhite)
	{
		glDeleteTextures(1, &g_gluiTextureWhite);
		g_gluiTextureWhite = 0;
	}
