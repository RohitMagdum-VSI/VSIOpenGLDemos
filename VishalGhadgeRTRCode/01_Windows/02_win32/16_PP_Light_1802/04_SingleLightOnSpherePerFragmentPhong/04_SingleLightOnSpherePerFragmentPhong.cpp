...
#define	CLASS_NAME		TEXT("PP : Single light on steady sphere -  Per fragment phong")
...
VOID initialize()
{
	...
	/*
	//Log  Sphere data to text file.

	#define ARRAY_SIZE(X) sizeof(X)/sizeof(X[0])

	fprintf(g_fpLogFile, "\nSphere vertices:");
	for (int i = 0; i < ARRAY_SIZE(g_farrSphereVertices); i++)
	{
		fprintf(g_fpLogFile, "%f,", g_farrSphereVertices[i]);
	}

	fprintf(g_fpLogFile, "\nSphere normals:");
	for (int i = 0; i < ARRAY_SIZE(g_farrSphereNormals); i++)
	{
		fprintf(g_fpLogFile, "%f,", g_farrSphereNormals[i]);
	}

	fprintf(g_fpLogFile, "\nSphere g_farrSphereTextures:");
	for (int i = 0; i < ARRAY_SIZE(g_farrSphereTextures); i++)
	{
		fprintf(g_fpLogFile, "%f,", g_farrSphereTextures[i]);
	}

	fprintf(g_fpLogFile, "\nSphere g_uiarrSphereElements:");
	for (int i = 0; i < ARRAY_SIZE(g_uiarrSphereElements); i++)
	{
		fprintf(g_fpLogFile, "%f,", g_uiarrSphereElements[i]);
	}

	fprintf(g_fpLogFile, "\n g_gluiNumVertices:%d", g_gluiNumVertices);
	fprintf(g_fpLogFile, "\n g_gluiNumElements:%d", g_gluiNumElements);
	*/
