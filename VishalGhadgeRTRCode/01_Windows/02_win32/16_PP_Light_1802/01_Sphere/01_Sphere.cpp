...
#define	CLASS_NAME		TEXT("PP : Sphere")
...
VOID initialize()
{
	...
	g_iNumVertices = getNumberOfSphereVertices();
	g_iNumElements = getNumberOfSphereElements();
	...
	glBindBuffer(GL_ARRAY_BUFFER, g_gluiVBOElement);

	glBufferData(GL_ARRAY_BUFFER, sizeof(sphere_elements), sphere_elements, GL_STATIC_DRAW);

	glVertexAttribPointer(RTR_ATTRIBUTE_ELEMENT, 3, GL_FLOAT, GL_FALSE, 0, NULL);

	glEnableVertexAttribArray(RTR_ATTRIBUTE_ELEMENT);
