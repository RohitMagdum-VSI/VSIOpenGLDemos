...
GLuint	g_gluiTexture;

#define	CLASS_NAME		TEXT("04: Checker Board")

GLubyte arrCheckImage[IMAGE_HEIGHT][IMAGE_WIDTH][4];
...
BOOLEAN LoadGLTextures()
{
	...
	glGenTextures(1, &g_gluiTexture);

	glBindTexture(GL_TEXTURE_2D, g_gluiTexture);	//	Bind texture.
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, IMAGE_WIDTH, IMAGE_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, arrCheckImage);
...
void MakeCheckImage()
{
	...
	for (i = 0; i < IMAGE_HEIGHT; i++)
	{
		for (j = 0; j < IMAGE_WIDTH; j++)
		{
			...
			arrCheckImage[i][j][0] = (GLubyte)c;
			arrCheckImage[i][j][1] = (GLubyte)c;
			arrCheckImage[i][j][2] = (GLubyte)c;
			arrCheckImage[i][j][3] = (GLubyte)c;
		}
	}
}
...
VOID resize(int iWidth, int iHeight)
{
	...
	if (iWidth <= iHeight)
	{
		gluPerspective(60, (GLfloat)iHeight / (GLfloat)iWidth, 1.0f, 30.0f);
	}
	else
	{
		gluPerspective(60, (GLfloat)iWidth / (GLfloat)iHeight, 1.0f, 30.0f);
	}
...
VOID uninitialize()
{
	if (g_gluiTexture)
	{
		glDeleteTextures(1, &g_gluiTexture);
		g_gluiTexture = 0;
	}
