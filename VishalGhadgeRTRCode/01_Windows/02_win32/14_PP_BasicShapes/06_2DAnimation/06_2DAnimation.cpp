...
#define	CLASS_NAME		TEXT("PP : 2D Animation")

#define LOG_FILE_NAME	("log.txt")
//	Handle to log file
FILE *g_fpLogFile = NULL;

GLfloat g_glfAngleTri = 0.0f;
GLfloat g_glfAngleRect = 0.0f;
...
VOID initialize()
{
	...
	glGenVertexArrays(1, &g_gluiVAOTriangle);
	...
	////////////////////////////////////////////////////////////////////
	//+	Square VAO
	glGenVertexArrays(1, &g_gluiVAORect);
...
void update()
{
	g_glfAngleTri = g_glfAngleTri + 0.1f;

	if (g_glfAngleTri >= 360)
	{
		g_glfAngleTri = 0.0f;
	}

	g_glfAngleRect = g_glfAngleRect + 0.1f;

	if (g_glfAngleRect >= 360)
	{
		g_glfAngleRect = 0.0f;
	}
}
...
VOID display()
{
	...
	matModelView = matModelView * rotate(g_glfAngleTri, 0.0f, 1.0f, 0.0f);
	...
	matModelView = matModelView * rotate(g_glfAngleRect, 1.0f, 0.0f, 0.0f);
