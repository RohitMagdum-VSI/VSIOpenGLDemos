...
#define	CLASS_NAME		TEXT("Ortho Multi Color Triangle")
...
VOID initialize()
{
	...
	//
	//	Resize.
	//	Removing comment so that projection matrix set as orthographic projection
	//	instead of by default perspective projection matrix.
	//
...
VOID resize(int iWidth, int iHeight)
{
	...
	//
	//	Change projection matrix to orthographic projection from perspective projection.
	//
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();

	//
	//	Viewing volume is of 100, though values are 50.
	//	-50 to +50 = 100
	//
	if (iWidth <= iHeight)
	{
		glOrtho(-50.0f, 50.0f, -50.0f * ((GLfloat)iHeight / (GLfloat)iWidth), 50.0f * ((GLfloat)iHeight / (GLfloat)iWidth), -50.0f, 50.0f);
	}
	else
	{
		glOrtho(-50.0f * ((GLfloat)iWidth / (GLfloat)iHeight), 50.0f * ((GLfloat)iWidth / (GLfloat)iHeight), -50.0f, 50.0f, -50.0f, 50.0f);
	}
