...
#define WINDOW_CAPTION	"Rotating Shapes 2D"
...
float g_fTriangleAngle = -360.0f;
float g_fRectAngle = -360.0f;
#define SPEED 1.0f
...
void display()
{
	...
	glRotatef(g_fTriangleAngle, 0.0f, 1.0f, 0.0f);
	MultiColorTriangle();
	...
	glTranslatef(3.0f, 0.0f, -8.0f);
	glRotatef(g_fRectAngle, 0.0f, 1.0f, 0.0f);	
	DrawSquare();
...
void update()
{
	if (g_fTriangleAngle <= 360.0f)
	{
		g_fTriangleAngle += SPEED;	
	}
	else
	{
		g_fTriangleAngle = -360.0f;
	}
	
	if (g_fRectAngle <= 360.0f)
	{
		g_fRectAngle += SPEED;	
	}
	else
