package com.astromedicomp.twoD_animation;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnGestureListener,OnDoubleTapListener{
	...
	private float g_fAngleTri;
	private float g_fAngleRect;
	...
	private void initialize(GL10 gl)
	{
		...
		byBuffer = ByteBuffer.allocateDirect(arrTriangleVertices.length * 4);
		byBuffer.order(ByteOrder.nativeOrder());
		fVerticesBuffer = byBuffer.asFloatBuffer();
		fVerticesBuffer.put(arrTriangleVertices);
		...
		//	Convert array to byte data.
		byBuffer = ByteBuffer.allocateDirect(farrTriangleColor.length * 4);
		byBuffer.order(ByteOrder.nativeOrder());
		fVerticesBuffer = byBuffer.asFloatBuffer();
		fVerticesBuffer.put(farrTriangleColor);
		fVerticesBuffer.position(0);
		
		GLES30.glBufferData(
							GLES30.GL_ARRAY_BUFFER,
							farrTriangleColor.length * 4,
							fVerticesBuffer,
							GLES30.GL_STATIC_DRAW
							);
		...
		GLES30.glVertexAttrib3f(GLESMacros.RTR_ATTRIBUTE_COLOR, 0.39f, 0.58f, 0.92f);
	...
	public void draw()
	{
		...
		Matrix.rotateM(modelViewMatrix, 0, g_fAngleTri,0.0f,1.0f,0.0f);
		...
		Matrix.rotateM(modelViewMatrix, 0, g_fAngleRect,1.0f,0.0f,0.0f);
	...
	private void update()
	{
		g_fAngleTri = g_fAngleTri + g_fAnimationSpeed;
		if (g_fAngleTri > 360)
		{
			g_fAngleTri = 0.0f;
		}
		
		g_fAngleRect = g_fAngleRect + g_fAnimationSpeed;
		if (g_fAngleRect > 360)
		{
			g_fAngleRect = 0.0f;
		}
