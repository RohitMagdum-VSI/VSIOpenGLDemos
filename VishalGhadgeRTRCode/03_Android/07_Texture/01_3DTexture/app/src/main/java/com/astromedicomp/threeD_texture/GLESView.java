...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnGestureListener,OnDoubleTapListener{
	...
	private void initialize(GL10 gl)
	{
		...
		final float arrPyramidTexCoord[] =
		{
			//	Front.
			0.5f,1.0f,
			0.0f,0.0f,
		...
		//	Convert array to byte data.
		byBuffer = ByteBuffer.allocateDirect(arrPyramidTexCoord.length * 4);
		byBuffer.order(ByteOrder.nativeOrder());
		fVerticesBuffer = byBuffer.asFloatBuffer();
		fVerticesBuffer.put(arrPyramidTexCoord);
		fVerticesBuffer.position(0);
		
		GLES30.glBufferData(
							GLES30.GL_ARRAY_BUFFER,
							arrPyramidTexCoord.length * 4,
							fVerticesBuffer,
							GLES30.GL_STATIC_DRAW
							);
		...
		//	Convert array to byte data.
		byBuffer = ByteBuffer.allocateDirect(arrCubeTextCoord.length * 4);
		byBuffer.order(ByteOrder.nativeOrder());
		fVerticesBuffer = byBuffer.asFloatBuffer();
		fVerticesBuffer.put(arrCubeTextCoord);
		fVerticesBuffer.position(0);
		
		GLES30.glBufferData(
							GLES30.GL_ARRAY_BUFFER,
							arrCubeTextCoord.length * 4,
							fVerticesBuffer,
							GLES30.GL_STATIC_DRAW
							);
