...
GLuint ShaderProgramObjectDeferredRendering;
GLuint ShaderProgramObjectGBuffer;

GLuint FinalQuadVAO, FinalQuadVBO;
void RenderQuad(void);
void ResizeFramebuffers(void);
...
void InitializeOpenGL(void)
{
	...
	ShaderInfo GBuffer[] = 
	{
		{ GL_VERTEX_SHADER, "Resources/Shaders/GBuffer.vert.glsl", 0 },
		{ GL_FRAGMENT_SHADER, "Resources/Shaders/GBuffer.frag.glsl", 0 },
		{ GL_NONE, NULL, 0 }
	};

	ShaderProgramObjectGBuffer = LoadShaders(GBuffer);

	ShaderInfo DeferredRendering[] = 
	{
		{ GL_VERTEX_SHADER, "Resources/Shaders/DeferredRendering.vert.glsl", 0 },
		{ GL_FRAGMENT_SHADER, "Resources/Shaders/DeferredRendering.frag.glsl", 0 },
		{ GL_NONE, NULL, 0 }
	};

	ShaderProgramObjectDeferredRendering = LoadShaders(DeferredRendering);
	...
	/////////////////////////////////////////////////////////////////
	// Creating framebuffers for deffered rendering
	glGenFramebuffers(1, &GeometryBuffer);
	glBindFramebuffer(GL_FRAMEBUFFER, GeometryBuffer);

	// Configuring the position color buffer
	glGenTextures(1, &PositionBufferTexture);
	glBindTexture(GL_TEXTURE_2D, PositionBufferTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB16F, giWinWidth, giWinHeight, 0, GL_RGB, GL_FLOAT, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, PositionBufferTexture, 0);

	// Configuring normal buffer
	...
	// Configuring albedo + specular color buffer
	glGenTextures(1, &AlbedoSpecularBufferTexture);
	glBindTexture(GL_TEXTURE_2D, AlbedoSpecularBufferTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, giWinWidth, giWinHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D, AlbedoSpecularBufferTexture, 0);

	glDrawBuffers(3, BufferAttchments);

	// create,configure and attch the depth buffer
	glGenRenderbuffers(1, &RBODepth);
	glBindRenderbuffer(GL_RENDERBUFFER, RBODepth);
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, giWinWidth, giWinHeight);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, RBODepth);

	// check for the framebuffer completeness
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
		MessageBox(ghWnd, TEXT("Incomplete deffered render framebuffer!"), TEXT("Framebuffer Error"), MB_ICONERROR | MB_OK);

	glBindFramebuffer(GL_FRAMEBUFFER, 0);

	glUseProgram(ShaderProgramObjectDeferredRendering);
	glUniform1i(glGetUniformLocation(ShaderProgramObjectDeferredRendering, "PositionTexture"), 0);
	glUniform1i(glGetUniformLocation(ShaderProgramObjectDeferredRendering, "NormalTexture"), 1);
	glUniform1i(glGetUniformLocation(ShaderProgramObjectDeferredRendering, "AlbedoSpecularTexture"), 2);
	glUseProgram(0);

	glUseProgram(ShaderProgramObjectGBuffer);
	glUniform1i(glGetUniformLocation(ShaderProgramObjectGBuffer, "material.DiffuseMap"), 0);
	glUniform1i(glGetUniformLocation(ShaderProgramObjectGBuffer, "material.SpecularMap"), 1);
	glUniform1i(glGetUniformLocation(ShaderProgramObjectGBuffer, "material.NormalMap"), 2);
	glUseProgram(0);
...
void RenderScene(void)
{
	...
	SetDayLight(ShaderProgramObjectDeferredRendering);
	RenderQuad();
...
void DrawAllBasicShapesInOneRowWithNormalMap(GLuint DiffuseTexture, GLuint SpecularTexture, GLuint NormalTexture, glm::mat4 ModelMatrix, glm::mat4 TranslationMatrix, glm::mat4 RotationMatrix, glm::mat4 ViewMatrix)
{
	...
	glUniformMatrix4fv(glGetUniformLocation(ShaderProgramObjectGBuffer, "u_view_matrix"), 1, GL_FALSE, glm::value_ptr(ViewMatrix));
	glUniformMatrix4fv(glGetUniformLocation(ShaderProgramObjectGBuffer, "u_projection_matrix"), 1, GL_FALSE, glm::value_ptr(gPerspectiveProjectionMatrix));

	//SetDayLightNormalMap(gShaderProgramObjectBasicShapesNormalMap);
	//SetDayLightNormalMap(ShaderProgramObjectGBuffer); -> this is done in lighting pass
	...
	//glUniform1f(glGetUniformLocation(gShaderProgramObjectBasicShapesNormalMap, "material.Shininess"), 128.0f); //-> thishas been set inside the shader
	...
	glUniformMatrix4fv(glGetUniformLocation(ShaderProgramObjectGBuffer, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCRing[1]->DrawRing();
...
void ResizeFramebuffers(void)
{
	...
	// Configuring the position color buffer
	glBindTexture(GL_TEXTURE_2D, PositionBufferTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, giWinWidth, giWinHeight, 0, GL_RGB, GL_FLOAT, NULL);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, PositionBufferTexture, 0);
