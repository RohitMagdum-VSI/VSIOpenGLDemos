...
////////////////////////////////////////////////////////
// basic shapes
CRingV *pCRingV[] = { NULL, NULL };
CSphereV *pCSphereV[] = { NULL, NULL };
CTorusV *pCTorusV[] = { NULL, NULL };
CCylinderV *pCCylinderV[] = { NULL, NULL };
CConeV *pCConeV[] = { NULL, NULL };

CSphereV *pCSkySphere = NULL;

// texture IDs for basic shapes
GLuint BrickDiffuse, BrickSpecular;
GLuint BrickDiffuseGranite, BrickDiffuseGrey, BrickDiffuseOldRed, BrickSpecular2;
GLuint GraniteDiffuse, GraniteSpecular;
GLuint PavingDiffuse, PavingSpecular;
GLuint PlasterDiffuseGreen, PlasterDiffuseWhite, PlasterDiffuseYellow, PlasterSpecular;
GLuint WoodPlatesDiffuse, WoodPlatesSpecular;
...
void InitializeOpenGL(void)
{
	...
	pCRingV[0] = new CRingV();
	pCRingV[1] = new CRingV();

	pCSphereV[0] = new CSphereV();
	pCSphereV[1] = new CSphereV();

	pCTorusV[0] = new CTorusV();
	pCTorusV[1] = new CTorusV();

	pCCylinderV[0] = new CCylinderV();
	pCCylinderV[1] = new CCylinderV();
	
	pCConeV[0] = new CConeV();
	pCConeV[1] = new CConeV();

	pCSkySphere = new CSphereV();
	...
	pCRingV[0]->InitializeRing(2.5f, 5.0f, 100);
	pCRingV[1]->InitializeRing(2.5f, 5.0f, 100);

	pCSphereV[0]->InitializeSphere(5.0f, 100, 100);
	pCSphereV[1]->InitializeSphere(5.0f, 100, 100);

	pCTorusV[0]->InitializeTorus(2.5f, 5.0f, 100, 100);
	pCTorusV[1]->InitializeTorus(2.5f, 5.0f, 100, 100);

	pCCylinderV[0]->InitializeCylinder(5.0f, 7.0f, 100, GL_TRUE, GL_TRUE);
	pCCylinderV[1]->InitializeCylinder(5.0f, 7.0f, 100, GL_TRUE, GL_TRUE);

	pCConeV[0]->InitializeCone(5.0f, 7.0f, 100, GL_TRUE);
	pCConeV[1]->InitializeCone(5.0f, 7.0f, 100, GL_TRUE);
	
	pCSkySphere->InitializeSphere(5.0f, 15, 15);
...
void RenderScene(void)
{
	...
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_view_matrix"), 1, GL_FALSE, glm::value_ptr(ViewMatrix));
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_projection_matrix"), 1, GL_FALSE, glm::value_ptr(gPerspectiveProjectionMatrix));

	SetDayLight(gShaderProgramBasicShapes);
	...
	glUniform1f(glGetUniformLocation(gShaderProgramBasicShapes, "material.Shininess"), 128.0f);

	// Ring 1
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[0]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCRingV[0]->DrawRing();

	// Ring 2
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[1]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCRingV[1]->DrawRing();
	...
	// Sphere 1
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[2]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCSphereV[0]->DrawSphere();

	// Sphere 2
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[3]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCSphereV[1]->DrawSphere();

	// Torus 1
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[4]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCTorusV[0]->DrawTorus();

	// Torus 2
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[5]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCTorusV[1]->DrawTorus();

	// Cylinder 1
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[6]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCCylinderV[0]->DrawCylinder();

	// Cylinder 2
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[7]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCCylinderV[1]->DrawCylinder();

	// Cone 1
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[8]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCConeV[0]->DrawCone();
	...
	// Cone 2
	ModelMatrix = glm::mat4(1.0f);
	TranslationMatrix = glm::translate(glm::mat4(1.0f), Positions[9]);
	ModelMatrix = ModelMatrix * TranslationMatrix;
	glUniformMatrix4fv(glGetUniformLocation(gShaderProgramBasicShapes, "u_model_matrix"), 1, GL_FALSE, glm::value_ptr(ModelMatrix));
	pCConeV[1]->DrawCone();
...
void Uninitialize(void)
{
	...
	delete pCCylinderV[0];
	delete pCCylinderV[1];
