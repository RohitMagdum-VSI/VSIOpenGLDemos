...
CSkyDome::CSkyDome()
{
	...
	OuterRadius = 10.25f;
*/
	InnerRadius = 10.0f * 1000.0f;
	OuterRadius = 10.25f * 1000.0f;
	...
	SunCPos = glm::rotate(glm::vec3(-512.0f, 0.0f, 0.0f), glm::radians(-22.5f), SunRotVec);
	...
	fprintf_s(pSkyFile, "SunCPos: %s\tSunWPos: %s\n", glm::to_string(SunCPos).c_str(), glm::to_string(SunWPos).c_str());
}
...
void CSkyDome::UpdateSky(GLfloat fDelta)
{
	...
	DiffuseIntensity = 1.75f * glm::min(1.0f, glm::max(0.0f, (0.03125f + SunCDir.y) / 0.0625f));
