...
CGrass::~CGrass()
{
	...
	delete[] ModelMatricesUpdateChunk;
	delete[] BrightnessValues;
	delete[] BrightnessValuesPtr;
	delete[] SidePtr;
	delete[] UpPtr;
	delete[] ForwardPtr;
	delete[] PosPtr;
	...
	if (VBOBrightness)
	{
		glDeleteBuffers(1, &VBOBrightness);
		VBOBrightness = 0;
	}

	if (VBOGrassColor)
	{
		glDeleteBuffers(1, &VBOGrassColor);
		VBOGrassColor = 0;
	}

	if (VBOGrassNormal)
	{
		glDeleteBuffers(1, &VBOGrassNormal);
		VBOGrassNormal = 0;
	}

	if (VBOGrassPosition)
	{
		glDeleteBuffers(1, &VBOGrassPosition);
		VBOGrassPosition = 0;
	}

	if (VBOModelMatrices)
	{
		glDeleteBuffers(1, &VBOModelMatrices);
		VBOModelMatrices = 0;
	}
	...
	if (ShaderProgramObjectGrass)
	{
		glDeleteProgram(ShaderProgramObjectGrass);
		ShaderProgramObjectGrass = 0;
	}

	if (ShaderProgramObjectGrassNight)
	{
		glDeleteProgram(ShaderProgramObjectGrassNight);
		ShaderProgramObjectGrassNight = 0;
	}
}
