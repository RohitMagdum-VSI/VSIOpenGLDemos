...
#define LOG_TAG "JGG: glOpenGLES32OrthoNative"
...
Java_com_jayshree_ndkoglortho_GLESNativeLib_resize(JNIEnv *env, jclass type, jint width, jint height)
{
    GLfloat aspectRatio;
    ...
    aspectRatio = width > height ? (GLfloat)width / (GLfloat)height : (GLfloat)height / (GLfloat)width;

    if (width <= height)
    {
        gOrthographicProjectionMatrix = glm::ortho(-100.0f, 100.0f, (-100.0f / aspectRatio), (100.0f / aspectRatio), -100.0f, 100.0f);
    }
    else
    {
        gOrthographicProjectionMatrix = glm::ortho((-100.0f * aspectRatio), (100.0f * aspectRatio), -100.0f, 100.0f, -100.0f, 100.0f);
    }
}

extern "C" JNIEXPORT void JNICALL
Java_com_jayshree_ndkoglortho_GLESNativeLib_init(JNIEnv *env, jclass type)
{
    printOpenGLESInfo();
...
extern "C" JNIEXPORT void JNICALL
Java_com_jayshree_ndkoglortho_GLESNativeLib_display(JNIEnv *env, jclass type)
{
    //code
