...
AAsset *assetKundali = NULL;
AAsset *assetStone = NULL;
...
GLint loadTexture(AAsset *assetFile)
{
    size_t textureDataSize = AAsset_getLength(assetFile);

    std::vector<uint8_t> fileBits;

    fileBits.resize(textureDataSize);

    AAsset_read(assetFile, fileBits.data(), fileBits.size());

    AAsset_close(assetFile);

    //read data into image properties

    uint32_t twidth, theight, tchannels;
    stbi_set_flip_vertically_on_load(true);
    uint8_t *image = stbi_load_from_memory(fileBits.data(), fileBits.size(), reinterpret_cast<int *>(&twidth), reinterpret_cast<int *>(&theight), reinterpret_cast<int *>(&tchannels), STBI_rgb);

    if (textureDataSize == fileBits.size())
        LOGINFO("NDK: Texture data parsed successfully");
...
Java_com_jayshree_ndkoglthreedtextureassetmanager_GLESNativeLib_readAssetsNative(JNIEnv *env, jclass type, jobject assetManager)
{
    AAssetManager *manager = AAssetManager_fromJava(env, assetManager);
    assetKundali = AAssetManager_open(manager, "kundali.png", AASSET_MODE_BUFFER);
    assetStone = AAssetManager_open(manager, "stone.png", AASSET_MODE_BUFFER);
}

extern "C" JNIEXPORT void JNICALL
Java_com_jayshree_ndkoglthreedtextureassetmanager_GLESNativeLib_resize(JNIEnv *env, jclass type, jint width, jint height)
{
    GLfloat aspectRatio;
...
extern "C" JNIEXPORT void JNICALL
Java_com_jayshree_ndkoglthreedtextureassetmanager_GLESNativeLib_init(JNIEnv *env, jclass type)
{
    printOpenGLESInfo();
    ...
    gTexture_Kundali = loadTexture(assetKundali);
    gTexture_Stone = loadTexture(assetStone);
...
extern "C" JNIEXPORT void JNICALL
Java_com_jayshree_ndkoglthreedtextureassetmanager_GLESNativeLib_display(JNIEnv *env, jclass type)
{
    //code
