...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szClassName, TEXT("Geometry Shader"), WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE, 100, 100, WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	if (hwnd == NULL)
	{
...
void initialize(void)
{
	...
	const GLchar *geometryShaderSourceCode =
		...
		"uniform mat4 u_mvp_matrix;" \
		"out vec4 out_color_geometry;" \
		"void main(void)" \
		"{" \
		"color_property[0].color = vec4(1.0,0.0,0.0,0.0);" \
		"color_property[1].color = vec4(0.0,1.0,0.0,0.0);" \
		"color_property[2].color = vec4(0.0,0.0,1.0,0.0);" \
		"for(int vertex=0; vertex < 3; vertex++)" \
		"{" \
		"gl_Position = u_mvp_matrix * (gl_in[vertex].gl_Position + vec4(0.0,1.0,0.0,0.0));" \
		"out_color_geometry = color_property[vertex].color;" \
		"EmitVertex();" \
	...
	const GLchar *fragmentShaderSourceCode =
		"#version 450 core"\
		"\n"\
		"in vec4 out_color_geometry;" \
		"out vec4 FragColor;"\
		"void main(void)"\
		"{"\
		"FragColor=out_color_geometry;"\
		"}";
