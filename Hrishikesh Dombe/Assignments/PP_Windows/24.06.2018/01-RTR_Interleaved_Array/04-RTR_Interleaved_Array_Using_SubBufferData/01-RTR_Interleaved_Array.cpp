...
void initialize(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(cubeVertices) + sizeof(cubeColor) + sizeof(cubeNormal) + sizeof(cubeTexcoords), NULL, GL_STATIC_DRAW);

	glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(cubeVertices), cubeVertices);

	glVertexAttribPointer(HAD_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (void *)0);
	...
	glVertexAttribPointer(HAD_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat),(void *)sizeof(cubeVertices));

	glEnableVertexAttribArray(HAD_ATTRIBUTE_COLOR);

	glBufferSubData(GL_ARRAY_BUFFER, sizeof(cubeVertices) + sizeof(cubeColor), sizeof(cubeNormal), cubeNormal);

	glVertexAttribPointer(HAD_ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), (void *) (sizeof(cubeVertices) + sizeof(cubeColor)));
	...
	glVertexAttribPointer(HAD_ATTRIBUTE_TEXTURE0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), (void *) (sizeof(cubeVertices) + sizeof(cubeColor) + sizeof(cubeNormal)));
