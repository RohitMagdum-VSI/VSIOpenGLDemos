...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"float4 diffuse_light : LIGHT;" \
		"float2 texcoord : TEXCOORD;" \
		"float4 color : COLOR;" \
		"};" \
		"vertex_output main(float3 pos : POSITION, float3 color : COLOR, float3 normal : NORMAL, float2 texturecoord : TEXCOORD)" \
		"{" \
		"vertex_output output;" \
		"if(u_lkeypressed == 1)" \
		"{" \
		"float4 eyeCoordinates = mul(worldViewMatrix,float4(pos,1.0f));" \
		"float3 tnorm = normalize(mul((float3x3)worldViewMatrix,normal));" \
		...
		"output.vertex = mul(mvpMatrix,float4(pos,1.0f));" \
	...
	const char *pixelShaderSourceCode =
		...
		"{" \
		"float4 final_color = input.diffuse_light;" \
		//"float4 texture_temp = ;" 
		"final_color = final_color * myTexture2D.Sample(mySamplerState,input.texcoord) * input.color;" \
		//"float final_color = input.diffuse_light;" 
		"return(final_color);" \
		"}";
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(float)*ARRAYSIZE(normal_Cube);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, 0, &gpID3D11Buffer_VertexBuffer_Cube_Normal);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	memcpy(mappedSubresource.pData, normal_Cube, sizeof(normal_Cube));
...
void display(void)
{
	...
	UINT offset_position = sizeof(float) * 0;
	UINT offset_color = sizeof(float) * 3;
	UINT offset_normal = sizeof(float) * 6;
	UINT offset_texcoord = sizeof(float) * 9;
	...
	/*************Cube*************/
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_Cube, &stride, &offset_position);
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_Cube, &stride, &offset_color);
	gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpID3D11Buffer_VertexBuffer_Cube, &stride, &offset_normal);
	gpID3D11DeviceContext->IASetVertexBuffers(3, 1, &gpID3D11Buffer_VertexBuffer_Cube, &stride, &offset_texcoord);
