...
HRESULT initialize(void)
{
	...
	const char *vertexShaderSourceCode =
		...
		"float4 color : COLOR;" \
		"};" \
		"vertex_output main(float4 pos : POSITION, float4 color : COLOR, float4 normal : NORMAL, float2 texturecoord : TEXCOORD)" \
		"{" \
	...
	const char *pixelShaderSourceCode =
		...
		"float4 texture_temp = myTexture2D.Sample(mySamplerState,input.texcoord);" \
		"final_color = final_color * texture_temp * input.color;" \
	...
	bufferDesc.ByteWidth = sizeof(Cube_VCNT);
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(cube_Color);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(cube_Normal);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	...
	memcpy(mappedSubresource.pData, cube_Normal, sizeof(cube_Normal));
	...
	ZeroMemory(&bufferDesc, sizeof(D3D11_BUFFER_DESC));
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	bufferDesc.ByteWidth = sizeof(cube_Texcoord);
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
...
void display(void)
{
	...
	UINT offset = 0;
	UINT offset_color = 3;
	UINT offset_normal = 6;
	UINT offset_texcoord = 9;
	...
	gpID3D11DeviceContext->IASetVertexBuffers(2, 1, &gpID3D11Buffer_VertexBuffer_Cube_Normal, &stride, &offset);
	gpID3D11DeviceContext->IASetVertexBuffers(3, 1, &gpID3D11Buffer_VertexBuffer_Cube_Texture, &stride_texture, &offset);
