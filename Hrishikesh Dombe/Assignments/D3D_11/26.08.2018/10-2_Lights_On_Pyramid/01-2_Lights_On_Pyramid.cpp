...
float lightPosition_Red[] = { 100.0f,100.0f,-100.0f,1.0f };
...
float lightPosition_Blue[] = { -100.0f,100.0f,-100.0f,1.0f };
...
HRESULT initialize(void)
{
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, 0, &gpID3D11Buffer_VertexBuffer_Pyramid_Normal);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Failed For Vertex Buffer Pyramid Normal.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device::CreateBuffer() Succeeded For Vertex Buffer Pyramid Normal.\n");
		fclose(gpFile);
	}

	ZeroMemory(&mappedSubresource, sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Pyramid_Normal, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource);
	memcpy(mappedSubresource.pData, pyramid_normals, sizeof(pyramid_normals));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Pyramid_Normal, 0);
