...
GLfloat Color_Arr[] = { 1.0f,1.0f,1.0f,/*White*/
						1.0f,0.0f,0.0f,/*Red*/
...
void display(void)
{
	...
	glLineWidth(3.0f);
	for (GLfloat j = 0.1f; j <= 1.1f; j = j + 0.1f)
	{
		glBegin(GL_LINE_LOOP);
		glColor3f(Color_Arr[k], Color_Arr[k+1], Color_Arr[k+2]);
		for (GLfloat i = 0.0f; i < circle_points; i++)
		{
			angle = 2 * PI * i / circle_points;
			glVertex2f(j*cos(angle), j*sin(angle));
		}
		glEnd();
		k = k + 3;
	}
