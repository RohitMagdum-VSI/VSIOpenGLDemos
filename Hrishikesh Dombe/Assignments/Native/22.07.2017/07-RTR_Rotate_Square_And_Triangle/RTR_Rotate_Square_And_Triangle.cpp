...
GLfloat gfAngle_Triangle = 0.0f, gfAngle_Square = 0.0f;
...
void display(void)
{
	...
	glRotatef(gfAngle_Triangle, 0.0f, 1.0f, 0.0f);
	...
	glTranslatef(1.5f, 0.0f, -6.0f);
	glRotatef(gfAngle_Square, 1.0f, 0.0f, 0.0f);
	glBegin(GL_QUADS);
...
void update(void)
{
	gfAngle_Triangle = gfAngle_Triangle + 0.1f;
	if (gfAngle_Triangle >= 360.0f)
		gfAngle_Triangle = 0.0f;

	gfAngle_Square = gfAngle_Square - 0.1f;
	if (gfAngle_Square <= -360.0f)
		gfAngle_Square = 0.0f;
}
