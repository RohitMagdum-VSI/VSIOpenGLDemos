...
#include"..\..\..\..\Utilities\Tga_Loader\Tga_Loader.h"
...
HRESULT initialize(void)
{
	...
	{
		gfVertices[i].position = XMFLOAT3(gTerrainModel[i].x,gTerrainModel[i].y,gTerrainModel[i].z);
	...
	// // bool result = tga_Texture_Loader.LoadTarga("./dirt01d.tga",height,width,tga_data);
	// // if(!result)
	// // {
	// // 	fopen_s(&gpFile, gszLogFileName, "a+");
	...
	// gpID3D11DeviceContext->UpdateSubresource(pID3D11Texture2D, 0, NULL, image->pixels, image->rowPitch, 0);
	// fopen_s(&gpFile, gszLogFileName, "a+");
	// fprintf_s(gpFile, "1.\n");
	// fclose(gpFile);
