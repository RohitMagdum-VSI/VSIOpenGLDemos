...
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer_Matrix = NULL;
ID3D11Buffer *gpID3D11Buffer_ConstantBuffer_Light = NULL;
...
HRESULT initialize(void)
{
	...
	bool Result = InitializeTerrain("setup.txt");
	if(Result != true)
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	for(i=0;i<giVertex_Count;i++)
	{
		...
		gfVertices[i].normal = XMFLOAT3(gTerrainModel[i].nx,gTerrainModel[i].ny,gTerrainModel[i].nz);
		gulIndices[i] = i;
	}
	...
	D3D11_BUFFER_DESC bufferDesc_ConstantBuffer_Matrix;
	ZeroMemory(&bufferDesc_ConstantBuffer_Matrix,sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ConstantBuffer_Matrix.Usage=D3D11_USAGE_DEFAULT;
	bufferDesc_ConstantBuffer_Matrix.ByteWidth=sizeof(CBUFFER_Matrix);
	bufferDesc_ConstantBuffer_Matrix.BindFlags=D3D11_BIND_CONSTANT_BUFFER;

	hr=gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer_Matrix,nullptr,&gpID3D11Buffer_ConstantBuffer_Matrix);
	if (FAILED(hr))
	{
		if (pID3DBlob_Error != NULL)
		{
	...
	gpID3D11DeviceContext->VSSetConstantBuffers(0,1,&gpID3D11Buffer_ConstantBuffer_Matrix);

	D3D11_BUFFER_DESC bufferDesc_ConstantBuffer_Light;
	ZeroMemory(&bufferDesc_ConstantBuffer_Light,sizeof(D3D11_BUFFER_DESC));
	bufferDesc_ConstantBuffer_Light.Usage=D3D11_USAGE_DEFAULT;
	bufferDesc_ConstantBuffer_Light.ByteWidth=sizeof(CBUFFER_Light);
	bufferDesc_ConstantBuffer_Light.BindFlags=D3D11_BIND_CONSTANT_BUFFER;

	hr=gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer_Light,nullptr,&gpID3D11Buffer_ConstantBuffer_Light);
	if (FAILED(hr))
	{
		if (pID3DBlob_Error != NULL)
		{
	...
	gpID3D11DeviceContext->PSSetConstantBuffers(0,1,&gpID3D11Buffer_ConstantBuffer_Light);
...
bool InitializeTerrain(char *filename)
{
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "CalculateNormlas() Failed.\n");
		fclose(gpFile);
		return false;
	}
...
bool CalculateNormals()
{
	...
	normals = new VectorType[(giTerrainHeight-1)*(giTerrainWidth-1)];
	if(!normals)
	{
		return false;
	}
	...
	for(j=0;j<(giTerrainHeight-1);j++)
	{
		...
		{
			index1 = ((j+1)*giTerrainWidth) + i;//Bottom left Vertex
			index2 = ((j+1)*giTerrainWidth) + (i+1);//Bottom right Vertex
			index3 = (j * giTerrainWidth) + i; //Upper left Vertex

			// Get three vertices from the face.
			vertex1[0] = gHeightMap[index1].x;
			vertex1[1] = gHeightMap[index1].y;
			vertex1[2] = gHeightMap[index1].z;

			vertex2[0] = gHeightMap[index2].x;
			vertex2[1] = gHeightMap[index2].y;
			vertex2[2] = gHeightMap[index2].z;

			vertex3[0] = gHeightMap[index3].x;
			vertex3[1] = gHeightMap[index3].y;
			vertex3[2] = gHeightMap[index3].z;
	...
	for(j=0;j<giTerrainHeight;j++)
	{
		for(i=0;i<giTerrainWidth;i++)
		{
			...
			if(((i-1)>=0)&&((j-1)>=0))
			{
				index = ((j-1)*(giTerrainWidth-1))+(i-1);
			...
			//Bottom right face
			if((i<(giTerrainWidth-1))&&((j-1)>=0))
			{
				index = ((j-1)*(giTerrainWidth-1))+i;
			...
			//Upper left face
			if(((i-1)>=0)&&(j<(giTerrainHeight-1)))
			{
				index = (j*(giTerrainWidth-1)) + (i-1);
			...
			//Upper right face
			if((i<(giTerrainWidth-1))&&(j<(giTerrainHeight-1)))
			{
				index = (j* (giTerrainWidth-1))+i;
			...
			//Get an index to the vertex location in the heightmap array
			index = (j*giTerrainWidth) + i;

			//Normalize the final shared normal for this vertex and store it in the height map array.
			gHeightMap[index].nx = (sum[0]/length);
			gHeightMap[index].ny = (sum[1]/length);
			gHeightMap[index].nz = (sum[2]/length);
		}
	}
...
bool BuildTerrainModel()
{
	...
	for(j=0;j<(giTerrainHeight-1);j++)
	{
		for(i=0;i<(giTerrainWidth-1);i++)
		{
			...
			gTerrainModel[index].nx = gHeightMap[index1].nx;
			gTerrainModel[index].ny = gHeightMap[index1].ny;
			gTerrainModel[index].nz = gHeightMap[index1].nz;
			index++;
			...
			gTerrainModel[index].nx = gHeightMap[index2].nx;
			gTerrainModel[index].ny = gHeightMap[index2].ny;
			gTerrainModel[index].nz = gHeightMap[index2].nz;
			index++;
			...
			gTerrainModel[index].nx = gHeightMap[index3].nx;
			gTerrainModel[index].ny = gHeightMap[index3].ny;
			gTerrainModel[index].nz = gHeightMap[index3].nz;
			index++;
			...
			gTerrainModel[index].nx = gHeightMap[index4].nx;
			gTerrainModel[index].ny = gHeightMap[index4].ny;
			gTerrainModel[index].nz = gHeightMap[index4].nz;
			index++;
		}
	}
	return true;
...
void display(void)
{
	...
	CBUFFER_Matrix constantBuffer_Matrix;
	constantBuffer_Matrix.WorldMatrix = worldMatrix;
	constantBuffer_Matrix.ViewMatrix = viewMatrix;
	constantBuffer_Matrix.ProjectionMatrix = gPerspectiveProjectionMatrix;

	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_Matrix,0,NULL,&constantBuffer_Matrix,0,0);

	CBUFFER_Light constantBuffer_Light;
	constantBuffer_Light.diffuseColor = XMFLOAT4(1.0f,1.0f,1.0f,1.0f);
	constantBuffer_Light.lightDirection = XMFLOAT3(-0.5f,-1.0f,-0.5f);
	constantBuffer_Light.padding = 0.0f;

	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_Light,0,NULL,&constantBuffer_Light,0,0);
...
void uninitialize(void)
{
	...
	if(gpID3D11Buffer_ConstantBuffer_Matrix)
	{
		gpID3D11Buffer_ConstantBuffer_Matrix->Release();
		gpID3D11Buffer_ConstantBuffer_Matrix = NULL;
	}

	if(gpID3D11Buffer_ConstantBuffer_Light)
	{
		gpID3D11Buffer_ConstantBuffer_Light->Release();
		gpID3D11Buffer_ConstantBuffer_Light = NULL;
	}
