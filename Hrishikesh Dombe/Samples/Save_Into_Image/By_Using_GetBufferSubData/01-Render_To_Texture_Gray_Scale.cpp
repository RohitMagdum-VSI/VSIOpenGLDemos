...
void create_image_file(void)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, gVbo_Pos);
	glGetBufferSubData(GL_TEXTURE_BUFFER, 0, viewport[2] * viewport[3] * 3, image_data);
	sprintf(str, "Error Code is : %d", glGetError());
	MessageBox(NULL, str, TEXT("ERROR"), MB_OK);
	glMapBuffer(GL_TEXTURE_BUFFER, GL_READ_BUFFER);
