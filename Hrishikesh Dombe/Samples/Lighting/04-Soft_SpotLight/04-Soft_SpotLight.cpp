...
GLuint gVao_Sphere, gVao_Square, gVao_Cube;
...
GLuint gLightPositionUniform, gLightDirectionUniform;
GLuint gCutOffUniform, gOuterCutOffUniform;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_ADD:
			cutoff_angle = cutoff_angle + 1.0f;
			outer_cutoff_angle = outer_cutoff_angle + 1.0f;
			break;
...
void initialize(void)
{
	...
	gCutOffUniform = glGetUniformLocation(gShaderProgramObject, "u_cutOff");
	gOuterCutOffUniform = glGetUniformLocation(gShaderProgramObject, "u_outerCutOff");
...
void display(void)
{
	...
	if (gbLight == true)
	{
		...
		glUniform3fv(gLightPositionUniform, 1, lightPosition);
		...
		glUniform3fv(gViewPositionUniform, 1, vec3(0.0f,0.0f,3.0f));
		glUniform1f(gConstantUniform, 1.0f);
		glUniform1f(gLinearUniform, 0.09f);
		glUniform1f(gQuadraticUniform, 0.032f);
		glUniform3fv(gLightDirectionUniform, 1, vec3(0.0f, -1.0f, 0.0f));
		glUniform1f(gCutOffUniform, cos(vmath::radians(cutoff_angle)));
		glUniform1f(gOuterCutOffUniform, cos(vmath::radians(outer_cutoff_angle)));
	}
	else
	{
	...
	scaleMatrix = scale(5.0f, 1.0f, 5.0f);
	...
	scaleMatrix = scale(0.04f, 0.04f, 0.04f);
	modelMatrix = modelMatrix * scaleMatrix;
